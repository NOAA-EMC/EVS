...
if [ $run_mpi = yes ] ; then
  export LD_LIBRARY_PATH=/apps/dev/pmi-fix:$LD_LIBRARY_PATH
   mpiexec -np 28 -ppn 28 --cpu-bind verbose,depth cfp run_all_poe.sh
else
 sh run_all_poe.sh
fi
...
tar -cvf plots.naefs.grid2obs.v${VDATE}.past${past_days}days.tar *.png

cp plots.naefs.grid2obs.v${VDATE}.past${past_days}days.tar  $COMOUT/.  
