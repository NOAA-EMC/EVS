...
while [ $n -le $past_days ] ; do
  ...
  echo $day
  sh $USHevs/global_ens/evs_get_gens_atmos_stat_file_link.sh $day "$model_list"
  n=$((n+1))
done 
  ...
  for lead in $fcst_leads ; do 
   ...
   if [ $lead = vs_lead ] ; then
	...
	OBS_LEVEL_value=$FCST_LEVEL_value
      ...
      for line_type in $line_tp ; do 
       ...
       echo "export field=${var}_${level}" >> run_${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh
        ...
        if [ $line_type = ctc ] ; then
          ...
          stat_list="acc"
	elif [ $line_type = ecnt ] ; then
          #stat_list="crps, crpss, rmse, spread, me"
          stat_list="rmse, spread"
        fi

        echo "export vx_mask_list='$VX_MASK_LIST'" >> run_${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh
        echo "export verif_case=$verif_case" >> run_${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh
        echo "export verif_type=$verif_type" >> run_${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh

        echo "export log_level=DEBUG" >> run_${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh
        echo "export met_ver=10.0" >> run_${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh
        #echo "export MODEL=$models" >> run_${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh
	 ...
	 thresh=' '
         sed -e "s!model_list!$models!g" -e "s!stat_list!$stat_list!g"  -e "s!thresh_list!$thresh!g"  -e "s!fcst_init_hour!$fcst_init_hour!g" -e "s!fcst_valid_hour!$fcst_valid_hour!g" -e "s!fcst_lead!$fcst_lead!g"  $USHevs/global_ens/evs_gens_atmos_grid2grid_plots.sh > run_py.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh
 ...
 for lead in $leads ; do
   ...
   for domain in g003 nhem shem tropics conus ; do
    ...
    for var in hgt tmp ugrd vgrd prmsl ; do
	 ...
	 mv ${score_type}_regional_${domain}_valid_00z_12z_${level}${unit}_${var}_${score}${lead}  evs.global_ens.${score}.${var}.${plevel}.last${past_days}days.${scoretype}_${valid_time}.${init_time}${lead_time}.${domain}_glb.png	 
