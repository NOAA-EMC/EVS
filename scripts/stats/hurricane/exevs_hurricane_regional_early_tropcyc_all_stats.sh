#!/bin/bash
set -x
export PS4=' + exevs_hurricane_regional_early_tropcyc_all_stats.sh line $LINENO: '
...
grep "03, HWFI" tracks.atcfunix.${YY23}_${stormBasin}${stormNumber} > a${stormBasin}${stormNumber}${stormYear}.dat
grep "03, HMNI" tracks.atcfunix.${YY23}_${stormBasin}${stormNumber} >> a${stormBasin}${stormNumber}${stormYear}.dat
...
sed -i 's/03, AVNI/03, MD01/' a${stormBasin}${stormNumber}${stormYear}.dat
sed -i 's/03, HWFI/03, MD02/' a${stormBasin}${stormNumber}${stormYear}.dat
sed -i 's/03, HMNI/03, MD03/' a${stormBasin}${stormNumber}${stormYear}.dat
sed -i 's/03, CTCI/03, MD04/' a${stormBasin}${stormNumber}${stormYear}.dat
sed -i 's/03, HFAI/03, MD05/' a${stormBasin}${stormNumber}${stormYear}.dat
sed -i 's/03, HFBI/03, MD06/' a${stormBasin}${stormNumber}${stormYear}.dat
sed -i 's/03, OFCL/03, MD07/' a${stormBasin}${stormNumber}${stormYear}.dat
export Model_List="MD01,MD02,MD03,MD04,MD05,MD06,MD07"
#export Model_Plot="GFS,HWRF,HMON,CTCI,HFSA,HFSB,OFCL"
