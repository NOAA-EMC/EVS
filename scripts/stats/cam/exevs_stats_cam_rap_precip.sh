#!/bin/sh
###############################################################################
# Name of Script: exevs_stats_cam_rap_precip.sh 
...
python $USHevs/${COMPONENT}/${COMPONENT}_precip_stats_get_data.py
...
for group in $JOB_GROUP_list; do
    ...
    python $USHevs/${COMPONENT}/${COMPONENT}_precip_stats_create_job_scripts.py
    ...
    if [ $JOB_GROUP = gather_stats ]; then
        ...
        python $USHevs/${COMPONENT}/${COMPONENT}_copy_job_dir_output.py
        export err=$?; err_chk
