...
export obs_lsr_file=${RUN}.${REP_DATE}/spc_lsr.${REP_DATE}/spc.lsr.${REP_DATE}12_${VDATE}12.G211.nc
export obs_ppf_file=${RUN}.${REP_DATE}/spc_lsr.${REP_DATE}/spc.ppf.${REP_DATE}12_${VDATE}12.G211.nc
...
while [ $fhr -le $fhr_max ]; do
   ...
   export fcst_file=${RUN}.${IDATE}/${modsys}.${IDATE}/${MODELNAME}.t${INIT_HR}z.MXUPHL25_A24.SSPF.${ACCUM_BEG}-${ACCUM_END}.f${fhr}.nc
   export MODEL_INPUT_TEMPLATE=${RUN}.{init?fmt=%Y%m%d}/${modsys}.{init?fmt=%Y%m%d}/${MODELNAME}.t{init?fmt=%2H}z.MXUPHL25_A24.SSPF.${ACCUM_BEG}-${ACCUM_END}.f{lead?fmt=%2H}.nc
