...
if [ $verif_spcoutlook = yes ] ; then
  ...
  export err=$?; err_chk
  cat ${DATA}/scripts/run_all_refs_spcoutlook_poe.sh >> $DATA/scripts/run_refs_all_grid2obs_poe
fi
...
if [ -s $DATA/scripts/run_refs_all_grid2obs_poe ] ; then
 ...
 mpiexec -np 2 -ppn 2 --cpu-bind verbose,core cfp  ${DATA}/scripts/run_refs_all_grid2obs_poe
 export err=$?; err_chk
fi
