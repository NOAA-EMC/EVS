...
export verif_system=${verif_system:-'yes'}
export verif_profile=${verif_profile:-'yes'}
export verif_product=${verif_product:-'yes'}
...
if [ $prepare = yes ] ; then
  ...
  if [ -s $COMINobsproc/rap.${VDATE}/rap.t12z.prepbufr.tm00 ] && [ -s $COMINobsproc/gdas.${vday}/00/atmos/gdas.t00z.prepbufr ] ; then
     ...
     export err=$?; err_chk
     $USHevs/cam/evs_refs_prepare.sh gfs_prepbufr $domain
     export err=$?; err_chk
...
#****************************************
>$DATA/scripts/run_refs_all_grid2obs_poe

if [ $verif_system = yes ] ; then 
  $USHevs/cam/evs_refs_grid2obs_system.sh 
  export err=$?; err_chk
  cat ${DATA}/scripts/run_all_refs_system_poe.sh >> $DATA/scripts/run_refs_all_grid2obs_poe
fi

if [ $verif_profile = yes ] ; then 
  $USHevs/cam/evs_refs_grid2obs_profile.sh $domain
  export err=$?; err_chk
  cat ${DATA}/scripts/run_all_refs_profile_poe.sh >> $DATA/scripts/run_refs_all_grid2obs_poe 
fi 

if [ $verif_product = yes ] ; then
  $USHevs/cam/evs_refs_grid2obs_product.sh
  export err=$?; err_chk
  cat ${DATA}/scripts/run_all_refs_product_poe.sh >> $DATA/scripts/run_refs_all_grid2obs_poe
fi


#totall: 72 jobs for all (both conus and alaska, profile, system and product)
chmod 775 $DATA/scripts/run_refs_all_grid2obs_poe

#*************************************************
# Run the POE script to generate small stat files
#*************************************************
if [ -s $DATA/scripts/run_refs_all_grid2obs_poe ] ; then
    #mpiexec -np 72 -ppn 72 --cpu-bind verbose,depth cfp  ${DATA}/scripts/run_refs_all_grid2obs_poe
    mpiexec -np 112 -ppn 112 --cpu-bind verbose,depth cfp  ${DATA}/scripts/run_refs_all_grid2obs_poe
    export err=$?; err_chk
fi
