...
#*************************************************
if [ -s ${DATA}/scripts/run_all_refs_precip_poe.sh ] || [ -s ${DATA}/scripts/run_all_refs_snowfall_poe.sh ] ; then

   if [ $verif_snowfall = no ] && [ $verif_precip = yes ] ; then
    mpiexec  -n 72 -ppn 72 --cpu-bind verbose,depth cfp ${DATA}/scripts/run_all_refs_precip_poe.sh
   elif [ $verif_snowfall = yes ] && [ $verif_precip = yes ] ; then
    mpiexec  -n 72 -ppn 72 --cpu-bind verbose,depth cfp ${DATA}/scripts/run_all_refs_precip_poe.sh
    mpiexec  -n 21 -ppn 21 --cpu-bind verbose,depth cfp ${DATA}/scripts/run_all_refs_snowfall_poe.sh
