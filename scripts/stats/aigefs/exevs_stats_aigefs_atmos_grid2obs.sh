#!/bin/ksh
#************************************************************************************
# Purpose: 1. Set up environment parameters for grid2obs job that are not
#             defined in stats J-job
#          2. Run grid2obs verification for gefs, aigefs, and hgefs
#
...
if [ $all_prepbufr_av = YES ]; then
  ...
  $USHevs/${COMPONENT}/evs_${COMPONENT}_atmos_${verif_case}.sh $ens
  export err=$?; err_chk
fi
