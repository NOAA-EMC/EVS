...
for MODEL in $MODELNAME; do
    ...
    if [ $MODEL == "gfs" ]; then
        ...
        for ihr in ${init_hours} ; do
            for hr in ${lead_hours} ; do
                COMINfilename="${COMINgfs}/${ihr}/wave/gridded/gfswave.t${ihr}z.global.0p25.f${hr}.grib2"
                DATAfilename="${DATA}/gfswave.${INITDATE}.t${ihr}z.global.0p25.f${hr}.grib2"
                COMOUTfilename="$COMOUT.${INITDATE}/${MODEL}/gfswave.${INITDATE}.t${ihr}z.global.0p25.f${hr}.grib2"
                if [ ! -s $COMINfilename ] ; then
                    if [ $SENDMAIL = YES ] ; then
                        export subject="F${hr} GFS Forecast Data Missing for EVS ${COMPONENT}"
                        echo "Warning: No GFS forecast was available for ${INITDATE}${ihr}f${hr}" > mailmsg
