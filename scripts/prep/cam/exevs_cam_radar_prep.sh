...
for DOMAIN in ${DOMAINS}; do
   ...
   for RADAR_FIELD in ${RADAR_FIELDS}; do
	 ...
	 export OBS_VAR=MergedReflectivityQCComposite
      ...
      if [ -s $DATA/MRMS_${DOMAIN}_tmp/${MRMS_PRODUCT}_${INITDATE}-${vhr}0000.grib2 ]; then
	 ...
	 # Copy output to $COMOUT
         ...
         if [ $SENDMAIL = YES ]; then
            ...
            echo "WARNING: The ${MRMS_PRODUCT} file is missing for valid date ${INITDATE}${vhr}. METplus will not run." > mailmsg
