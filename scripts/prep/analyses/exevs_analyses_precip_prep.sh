...
for NEST in "conus"; do
    ...
    for ACC in "01"; do
        ...
        for OBS_ACC in $OBS_ACCs; do
            ...
            python $USHevs/analyses/analyses_check_settings.py
            export err=$?; err_chk
     
            # Create Output Directories
            python $USHevs/analyses/analyses_create_output_dirs.py
            export err=$?; err_chk
     
            # Create Job Script 
            python $USHevs/analyses/analyses_prep_precip_create_job_script.py
            export err=$?; err_chk
            export njob=$((njob+1))
        done
    done
...
if [ $USE_CFP = YES ]; then
    python $USHevs/analyses/analyses_prep_precip_create_poe_job_scripts.py
    export err=$?; err_chk
fi

# Create Working Directories
python $USHevs/analyses/analyses_create_child_workdirs.py
export err=$?; err_chk
