...
status=$?
[[ $status -ne 0 ]] && exit $status
[[ $status -eq 0 ]] && echo "Successfully ran cam_plots_snowfall_create_job_scripts.py"
export njob=$((njob+1))
...
if [ $USE_CFP = YES ]; then
    ...
    status=$?
    [[ $status -ne 0 ]] && exit $status
    [[ $status -eq 0 ]] && echo "Successfully ran cam_plots_snowfall_create_poe_job_scripts.py"
fi
...
if [ $SENDCOM = YES ]; then
    find ${DATA}/${VERIF_CASE}/out/*/*/*.png -type f -print | tar -cvf ${COMOUTplots}/${NET}.${STEP}.${COMPONENT}.${RUN}.${VERIF_CASE}.v${VDATE}.tar -T -
fi
