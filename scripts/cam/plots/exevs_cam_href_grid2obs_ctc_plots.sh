    ...
    for VAR in $VARs ; do 
	  ...
	  FCST_LEVEL_values="ML"
       ...
       if [ $VAR = CAPEsfc ] || [ $VAR = MLCAPE ] ; then
         VX_MASK_LIST="CONUS, CONUS_East, CONUS_West, CONUS_South, CONUS_Central,  Appalachia, DeepSouth, GreatBasin, Mezquital, MidAtlantic,  PacificNW, Southeast, SPlains"
       else
...
if [ $run_mpi = yes ] ; then
  export LD_LIBRARY_PATH=/apps/dev/pmi-fix:$LD_LIBRARY_PATH
   mpiexec -np 15 -ppn 15 --cpu-bind verbose,depth cfp run_all_poe.sh
else
 sh run_all_poe.sh
fi
...
tar -cvf plots.href.grid2obs.ctc.v${VDATE}.past${past_days}days.tar *.png

cp  plots.href.grid2obs.ctc.v${VDATE}.past${past_days}days.tar  $COMOUT/.  
