#!/bin/bash
################################################################################
# Name of Script: exevs_nfcens_grid2obs_stats.sh                                
...
for theDate in ${date_list}; do
  cp ${ARCmodel}/${RUN}.${theDate}/${MODELNAME}/${VERIF_CASE}/HTSGW_mean*f???.grib2 ${DATA}/gribs/.
done
...
echo " Found ${nc} ${DATA}/gribs/HTSGW_mean*grib2 files for ${VDATE} "
if [ "${nc}" != '0' ]
then
  set +x
  echo "Successfully copied the ind fcst NFCENS grib2 files for ${VDATE}"
  [[ "$LOUD" = YES ]] && set -x
else
  set +x
  echo ' '
  ...
  [[ "$LOUD" = YES ]] && set -x
  ./postmsg "$jlogfile" "FATAL ERROR : NO ind fcst NFCENS GRIB2 FILES for ${VDATE}"
  err_exit "FATAL ERROR: Did not copy the ind fcst NFCENS grib2 files for ${VDATE}"
fi
...
cycles='00 12'
fhrs='fhr1 fhr2 fhr3 fhr4 fhr5 fhr6'
