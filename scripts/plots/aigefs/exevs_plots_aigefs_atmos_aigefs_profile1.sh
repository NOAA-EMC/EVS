#!/bin/ksh
#*******************************************************************************
# Purpose: set up environment, paths, and run the aigefs profile1  
#          plotting python scripts
# Note:    The profile plots are split to 3 smaller scripts: profile1,2,3.
#          profile1 is for the time series and forecast mean plots
#
    ...
    for score_type in time_series lead_average ; do
      ...
      for lead in $fcst_leads ; do 
        ...
        for VAR in HGT TMP UGRD VGRD ; do 
          ...
          elif [ $VAR = TMP ] || [ $VAR = UGRD ] || [ $VAR = VGRD ] ; then
...
#*********************************************************************
if [ $run_mpi = yes ] ; then
  mpiexec -np 330 -ppn 33 --cpu-bind verbose,depth cfp ${DATA}/run_all_poe.sh
else
  ${DATA}/run_all_poe.sh
  export err=$?; err_chk
fi
	...
	for var in hgt tmp ugrd vgrd ; do
	    ...
	    for level in $levels ; do
                ...
                if [ -f "lead_average_regional_${domain}_valid_${ihr}_${level}mb_${var}_${stats}.png" ]; then
                    mv lead_average_regional_${domain}_valid_${ihr}_${level}mb_${var}_${stats}.png evs.${COMPONENT}.${evs_graphic_stats}.${var}_${plevel}.last${past_days}days.fhrmean_valid${ihr}_f384.g003_${domain_new}.png
                fi
		...
		for lead in 24 120 240 360; do
                    ...
                    if [ -f "time_series_regional_${domain}_valid_${ihr}_${level}mb_${var}_${stats}_f${lead}.png" ]; then
                        mv time_series_regional_${domain}_valid_${ihr}_${level}mb_${var}_${stats}_f${lead}.png evs.${COMPONENT}.${evs_graphic_stats}.${var}_${plevel}.last${past_days}days.timeseries_valid${ihr}_f${lead_new}.g003_${domain_new}.png
                    fi
	        done # lead
            done # level
        done # var
