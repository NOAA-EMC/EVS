#!/bin/ksh
#**************************************************************************
# Purpose: set up environment, paths, and run the aigefs precip
...
for stats in ets fbias crps me_mae rmse_spread bss fss ; do 
  if [ $stats = ets ] ; then
    stat_list='ets'
    line_tp='ctc'
	  ...
	  OBS_LEVEL_value=$FCST_LEVEL_value
	    ...
	    if [ $line_tp = pstd ] ; then
	      ...
	      thresh_fcst=$threshes
            ...
            sed -e "s!model_list!$models!g" -e "s!stat_list!$stat_list!g" -e "s!thresh_fcst!$thresh_fcst!g" -e "s!thresh_obs!$thresh_obs!g" -e "s!fcst_init_hour!$fcst_init_hour!g" -e "s!fcst_valid_hour!$fcst_valid_hour!g" -e "s!fcst_lead!$fcst_lead!g" -e "s!interp_pnts!$interp_pnt_config!g" $USHevs/${COMPONENT}/evs_gens_atmos_plots_config.sh > $WORKtask/run_py.${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_tp}_${interp_pnt}.sh
...
#*********************************************************************
if [ $run_mpi = yes ] ; then
  mpiexec -np 140 -ppn 70 --cpu-bind verbose,depth cfp ${DATA}/run_all_poe.sh
else
  ${DATA}/run_all_poe.sh
  export err=$?; err_chk
fi
	...
	for thresh in $threshs; do
		...
		if [ $stat = bss ]; then
		    if [ -f "lead_average_regional_${domain}_valid_12z_24h_apcp_24_ens_freq${thresh_graphic}_bss.png" ]; then
		        mv lead_average_regional_${domain}_valid_12z_24h_apcp_24_ens_freq${thresh_graphic}_bss.png evs.${COMPONENT}.${stat}${nbhrd_graphic}${thresh_graphic}.apcp_a24.last${past_days}days.fhrmean_valid12z_f384.g212_${evs_graphic_domain}.png
		    fi
   	        else
                    if [ -f "lead_average_regional_${domain}_valid_12z_24h_apcp_24_${stat}${nbhrd_graphic}${thresh_graphic}.png" ]; then
                        mv lead_average_regional_${domain}_valid_12z_24h_apcp_24_${stat}${nbhrd_graphic}${thresh_graphic}.png evs.${COMPONENT}.${evs_graphic_stats}${nbhrd_graphic}${thresh_graphic}.apcp_a24.last${past_days}days.fhrmean_valid12z_f384.g212_${evs_graphic_domain}.png
                    fi
                fi
		...
		for lead in 24 72 120 240; do
			...
			if [ -f "time_series_regional_${domain}_valid_12z_24h_apcp_24_ens_freq${thresh_graphic}_bss${lead_graphic}.png" ]; then
			    mv time_series_regional_${domain}_valid_12z_24h_apcp_24_ens_freq${thresh_graphic}_bss${lead_graphic}.png evs.${COMPONENT}.${stat}${nbhrd_graphic}${thresh_graphic}.apcp_a24.last${past_days}days.timeseries_valid12z${lead_graphic}.g212_${evs_graphic_domain}.png
			fi
                    else
                        if [ -f "time_series_regional_${domain}_valid_12z_24h_apcp_24_${stat}${nbhrd_graphic}${lead_graphic}${thresh_graphic}.png" ]; then
                            mv time_series_regional_${domain}_valid_12z_24h_apcp_24_${stat}${nbhrd_graphic}${lead_graphic}${thresh_graphic}.png evs.${COMPONENT}.${evs_graphic_stats}${nbhrd_graphic}${thresh_graphic}.apcp_a24.last${past_days}days.timeseries_valid12z${lead_graphic}.g212_${evs_graphic_domain}.png
                        fi
                    fi
                done # lead
            done # nbrhd
