#!/bin/ksh
#*******************************************************************************
# Purpose: set up environment, paths, and run the aigefs grid2obs 
...
for fcst_valid_hour in $fcst_valid_hours ; do
  ...
  for stats in acc me_mae crpss rmse_spread ; do 
    if [ $stats = acc ] ; then
      stat_list='acc'
      line_tp='sal1l2'
      VARs='PRMSL TMP2m UGRD10m VGRD10m'
    ...
    for score_type in $score_types ; do
      ...
      for lead in $fcst_leads ; do 
	    ...
	    OBS_LEVEL_value=$FCST_LEVEL_value
	      ...
	      echo "export verif_type=conus_sfc" >> run_${fcst_valid_hour}.${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.sh
...
#*********************************************************************
if [ $run_mpi = yes ] ; then
  mpiexec -np 160 -ppn 40 --cpu-bind verbose,depth cfp ${DATA}/run_all_poe.sh
else
  ${DATA}/run_all_poe.sh
  export err=$?; err_chk
fi
...
for ihr in 00z 12z ; do
  for var in prmsl tmp ugrd vgrd ; do
            ...
            for level in $levels ; do
                ...
                if [ $var = prmsl ] ; then
                    if [ -f "lead_average_regional_${domain}_valid_${ihr}_${var}_${stats}.png" ]; then
                        mv lead_average_regional_${domain}_valid_${ihr}_${var}_${stats}.png evs.${COMPONENT}.${evs_graphic_stats}.${var}_${evs_graphic_level}.last${past_days}days.fhrmean_valid${ihr}_f384.g003_${domain_new}.png
                    fi
                else
                    if [ -f "lead_average_regional_${domain}_valid_${ihr}_${level}${unit}_${var}_${stats}.png" ]; then
                        mv lead_average_regional_${domain}_valid_${ihr}_${level}${unit}_${var}_${stats}.png evs.${COMPONENT}.${evs_graphic_stats}.${var}_${evs_graphic_level}.last${past_days}days.fhrmean_valid${ihr}_f384.g003_${domain_new}.png
                    fi
                fi
                ...
                for lead in 24 120 240 360; do
                    ...
                    if [ $var = prmsl ] ; then
                        if [ -f "time_series_regional_${domain}_valid_${ihr}_${var}_${stats}_f${lead}.png" ]; then
                            mv time_series_regional_${domain}_valid_${ihr}_${var}_${stats}_f${lead}.png evs.${COMPONENT}.${evs_graphic_stats}.${var}_${evs_graphic_level}.last${past_days}days.timeseries_valid${ihr}_f${lead_new}.g003_${domain_new}.png
                        fi
                    else
                        if [ -f "time_series_regional_${domain}_valid_${ihr}_${level}${unit}_${var}_${stats}_f${lead}.png" ]; then
                            mv time_series_regional_${domain}_valid_${ihr}_${level}${unit}_${var}_${stats}_f${lead}.png evs.${COMPONENT}.${evs_graphic_stats}.${var}_${evs_graphic_level}.last${past_days}days.timeseries_valid${ihr}_f${lead_new}.g003_${domain_new}.png
                        fi
                    fi
                done # lead
            done # level
