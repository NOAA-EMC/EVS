  ...
  for score_type in $score_types ; do
   ...
   for lead in $fcst_leads ; do 
	...
	if [ $score_type = lead_average ] ; then
	 ...
	 echo "   >$all_plots/run_${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.${fcst_valid_hour}.completed" >> run_${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.${fcst_valid_hour}.sh
	 ...
	 echo "if [ -s $all_plots/run_${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.${fcst_valid_hour}.completed ] ; then" >> run_${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.${fcst_valid_hour}.sh
...
if [ -s evs*.png ] ; then
 tar -cvf evs.plots.href.profile.last${last_days}days.v${VDATE}.tar evs*.png
fi
...
if [ $SENDCOM = YES ] && [ -s evs.plots.href.profile.last${last_days}days.v${VDATE}.tar ] ; then
 cp -v evs.plots.href.profile.last${last_days}days.v${VDATE}.tar  $COMOUT/.  
fi

if [ $SENDDBN = YES ] ; then
    $DBNROOT/bin/dbn_alert MODEL EVS_RZDM $job $COMOUT/evs.plots.href.profile.last${last_days}days.v${VDATE}.tar
fi
