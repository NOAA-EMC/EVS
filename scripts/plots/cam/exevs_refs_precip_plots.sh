   ...
   for domain in conus conus_east conus_west conus_south conus_central alaska  ; do
    ...
    for valid in $valids ; do
      ...
      if [ -s ${score_type}_regional_${domain}_valid_${valid}_${level}_${var}_${stats}_${lead}.png ] ; then
        ...
        mv ${score_type}_regional_${domain}_valid_${valid}_${level}_${var}_${stats}_${lead}.png  evs.refs.${stats}.${var}h.last${past_days}days.${scoretype}_valid_${valid}.${new_domain}.png
      fi
    done
  done
    ...
    for valid in $valids ; do
     ...
     if [ $domain = alaska ] ; then
      ...
      if [ -s ${score_type}_regional_${domain}_valid_${valid}_${level}_${var}_${lead}.png ] ; then
       mv ${score_type}_regional_${domain}_valid_${valid}_${level}_${var}_${lead}.png  evs.refs.ctc.${var}h.last${past_days}days.${scoretype}_valid_${valid}.${new_domain}.png
      fi
...
tar -cvf evs.plots.refs.precip.past${past_days}days.v${VDATE}.tar *.png
...
if [ $SENDCOM = YES ] && [ -s evs.plots.refs.precip.past${past_days}days.v${VDATE}.tar ] ; then
 cp -v evs.plots.refs.precip.past${past_days}days.v${VDATE}.tar  $COMOUT/.  
fi
