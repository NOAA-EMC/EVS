...
stat_header="VERSION MODEL DESC            FCST_LEAD FCST_VALID_BEG  FCST_VALID_END  OBS_LEAD OBS_VALID_BEG   OBS_VALID_END   FCST_VAR FCST_UNITS FCST_LEV OBS_VAR OBS_UNITS
 OBS_LEV OBTYPE VX_MASK           INTERP_MTHD INTERP_PNTS FCST_THRESH OBS_THRESH COV_THRESH ALPHA LINE_TYPE"
...
##############################################################################################
#
#STEP 1: Split the big stat files into small stat files based on REGIONs and validation hours
#STEP 2: Run METDATAIO to reformat the small stat files
#STEP 3: Run METCALCPY to aggregate the small stat files
...
for REGION in $VX_MASK_LISTs ; do
  for VAR in APCP_01 APCP_03 APCP_24 ; do
    ...
    for valid in $fcst_valid_hours ; do
      ...
      metdataio_input_small=$metdataio_input_dir/${REGION}_${valid}_${VAR}
      mkdir -p $metdataio_input_small
	...
	if [ -s $big_stat ] ; then
	  >$metdataio_input_small/${MODEL}_${vday}.stat
	  echo "$stat_header" >> $metdataio_input_small/${MODEL}_${vday}.stat
	  grep " $REGION " $big_stat|grep "${vday}_${valid}0000 $VAR"|grep RHIST >> $metdataio_input_small/${MODEL}_${vday}.stat
	fi
      done
     done
    done
	...
	bukovski=$domain
     ...
     var=`echo $VAR | tr '[A-Z]' '[a-z]'`
      ...
      #for thresh in 12.700  ; do 
       ...
       sed -e "s!INPUT_FILENAME!$input_metcalcpy_filename!g" -e "s!OUTPUT_FILENAME!$output_metcalcpy_filename!g" -e "s!VAR!$VAR!g" -e "s!THRESHOLD!$thresh!g" -e "s!MODEL!REFS_PROB!g" $CONFIG/config.refs.reliability.metcalcpy.yaml >  ${metcalcpy_output_dir}/reliability_metcalcpy.${REGION}.${valid}.yaml

       echo "if [ -s $input_metcalcpy_filename ] ; then" >> run_${REGION}.${valid}.${VAR}.${thresh}.sh
       echo "  python  $METCALCPY_BASE/metcalcpy/agg_stat.py  ${metcalcpy_output_dir}/reliability_metcalcpy.${REGION}.${valid}.yaml"  >> run_${REGION}.${valid}.${VAR}.${thresh}.sh 
       echo "fi"  >> run_${REGION}.${valid}.${VAR}.${thresh}.sh

       input_metplotcpy_filename=$output_metcalcpy_filename
       #output_metplotpy_filename=${metplotpy_output_dir}/reliability_${VAR}.gt.${thresh}_pct_agg_stat.${REGION}.${valid}.png
       ...
       sed -e "s!INPUT_FILENAME!$input_metplotcpy_filename!g" -e "s!OUTPUT_FILENAME!$output_metplotpy_filename!g" -e "s!VAR!$VAR!g" -e "s!THRESHOLD!$thresh!g" -e "s!REGION!$REGION!g" -e "s!VALIDBEG!$velid_beg!g" -e "s!VALIDEND!$valid_end!g"  $CONFIG/config.refs.reliability.metplotpy.yaml >  ${metplotpy_output_dir}/reliability_metplotpy.${REGION}.${valid}.yaml

       echo "if [ -s $input_metplotcpy_filename ] ; then" >> run_${REGION}.${valid}.${VAR}.${thresh}.sh
       echo "  python  $METPLOTPY_BASE/metplotpy/plots/reliability_diagram/reliability.py ${metplotpy_output_dir}/reliability_metplotpy.${REGION}.${valid}.yaml" >> run_${REGION}.${valid}.${VAR}.${thresh}.sh
       echo "fi" >> run_${REGION}.${valid}.${VAR}.${thresh}.sh	 
