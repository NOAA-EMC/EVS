  ...
  for score_type in $score_types ; do
   ...
   for threshold in $thresholds ; do 
	...
	#Save for restart and tar files 
	 ...
	 echo "     >$all_plots/run_${stats}.${thresh}.${score_type}.${lead}.${VAR}.${dom}.${FCST_LEVEL_value}.${valid_time}.completed" >> run_${stats}.${thresh}.${score_type}.${lead}.${VAR}.${dom}.${FCST_LEVEL_value}.${valid_time}.sh
	...
	#Copy files to restart directory
	 ...
	 echo "     cp -v $all_plots/run_${stats}.${thresh}.${score_type}.${lead}.${VAR}.${dom}.${FCST_LEVEL_value}.${valid_time}.completed $restart" >> run_${stats}.${thresh}.${score_type}.${lead}.${VAR}.${dom}.${FCST_LEVEL_value}.${valid_time}.sh
...
if [ -s evs*.png ] ; then
 tar -cvf evs.plots.href.grid2obs.cape.last${last_days}days.v${VDATE}.tar evs*.png
fi
...
if [ $SENDCOM = YES ] && [ -s evs.plots.href.grid2obs.cape.last${last_days}days.v${VDATE}.tar ] ; then
 cp -v evs.plots.href.grid2obs.cape.last${last_days}days.v${VDATE}.tar  $COMOUT/.  
fi
