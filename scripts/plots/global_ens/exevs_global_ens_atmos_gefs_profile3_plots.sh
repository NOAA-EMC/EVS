...
#
# Updated: 09/19/2025 by L. Gwen Chen (lichuan.chen@noaa.gov)
...
#*********************************************************************
if [ $run_mpi = yes ] ; then
  mpiexec -np 120 -ppn 12 --cpu-bind verbose,depth cfp ${DATA}/run_all_poe.sh
else
  ${DATA}/run_all_poe.sh
  export err=$?; err_chk
fi
	...
	for var in hgt tmp ugrd vgrd rh ; do
	    ...
	    for lead in $leads ; do
                ...
                if [ -f "stat_by_level_regional_${domain}_valid_${ihr}_${var}_${stats}_f${lead}.png" ]; then
                    mv stat_by_level_regional_${domain}_valid_${ihr}_${var}_${stats}_f${lead}.png evs.global_ens.${evs_graphic_stats}.${var}_all.last${past_days}days.vertprof_valid${ihr}_f${lead_new}.g003_${domain_new}.png
                fi
            done # lead
        done # var
    done # domain
