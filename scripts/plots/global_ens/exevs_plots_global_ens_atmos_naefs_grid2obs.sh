...
#
# Updated: 09/04/2025 by L. Gwen Chen (lichuan.chen@noaa.gov)
...
#*********************************************************************
if [ $run_mpi = yes ] ; then
  mpiexec -np 64 -ppn 32 --cpu-bind verbose,depth cfp ${DATA}/run_all_poe.sh
else
  ${DATA}/run_all_poe.sh
  export err=$?; err_chk
fi
            ...
            for domain in $domains ; do	
                ...
                if [ $domain = nhem ] || [ $domain = shem ] || [ $domain = tropics ] || [ $domain = alaska ]; then
                    if [ -f "lead_average_regional_${domain}_valid_${ihr}_${level}_${var}_${stats}.png" ]; then
                        mv lead_average_regional_${domain}_valid_${ihr}_${level}_${var}_${stats}.png evs.naefs.${evs_graphic_stats}.${var}_${level_new}.last${past_days}days.fhrmean_valid${ihr}_f384.g003_${evs_graphic_domain}.png
                    fi
                else
                    if [ -f "lead_average_regional_${domain}_valid_${ihr}_${level}_${var}_${stats}.png" ]; then
                        mv lead_average_regional_${domain}_valid_${ihr}_${level}_${var}_${stats}.png evs.naefs.${evs_graphic_stats}.${var}_${level_new}.last${past_days}days.fhrmean_valid${ihr}_f384.g212_${evs_graphic_domain}.png
                    fi
                fi
            done # domain
        done # level
