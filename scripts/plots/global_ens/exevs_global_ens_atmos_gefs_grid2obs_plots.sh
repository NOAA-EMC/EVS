...
#
# Updated: 09/03/2025 by L. Gwen Chen (lichuan.chen@noaa.gov)
...
#*********************************************************************
if [ $run_mpi = yes ] ; then
  mpiexec -np 220 -ppn 22 --cpu-bind verbose,depth cfp ${DATA}/run_all_poe.sh
else
  ${DATA}/run_all_poe.sh
  export err=$?; err_chk
fi
            ...
            for level in $levels ; do
                ...
                if [ $var = prmsl ] ; then
                    if [ -f "lead_average_regional_${domain}_valid_${ihr}_${var}_${stats}.png" ]; then
                        mv lead_average_regional_${domain}_valid_${ihr}_${var}_${stats}.png evs.global_ens.${evs_graphic_stats}.${var}_${evs_graphic_level}.last${past_days}days.fhrmean_valid${ihr}_f384.g003_${domain_new}.png
                    fi
                else
                    if [ -f "lead_average_regional_${domain}_valid_${ihr}_${level}${unit}_${var}_${stats}.png" ]; then
                        mv lead_average_regional_${domain}_valid_${ihr}_${level}${unit}_${var}_${stats}.png evs.global_ens.${evs_graphic_stats}.${var}_${evs_graphic_level}.last${past_days}days.fhrmean_valid${ihr}_f384.g003_${domain_new}.png
                    fi
                fi
