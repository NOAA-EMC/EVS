...
#          profile1 is for the time series and forecast mean plots of
#          rmse_spread and crps.
#
# Updated: 09/17/2025 by L. Gwen Chen (lichuan.chen@noaa.gov)
...
for fcst_valid_hour in $fcst_valid_hours ; do
  ...
  for stats in crps rmse_spread ; do 
    ...
    for score_type in time_series lead_average ; do
      ...
      for lead in $fcst_leads ; do 
        ...
        for VAR in HGT TMP UGRD VGRD RH ; do 
          ...
          if [ $VAR = HGT ] ; then
            FCST_LEVEL_values="P1000 P500"
          elif [ $VAR = TMP ] || [ $VAR = UGRD ] || [ $VAR = VGRD ] || [ $VAR = RH ] ; then
...
#*********************************************************************
if [ $run_mpi = yes ] ; then
  mpiexec -np 280 -ppn 20 --cpu-bind verbose,depth cfp ${DATA}/run_all_poe.sh
else
  ${DATA}/run_all_poe.sh
  export err=$?; err_chk
fi
	...
	for var in hgt tmp ugrd vgrd rh ; do
	    ...
	    for level in $levels ; do
                ...
                if [ -f "lead_average_regional_${domain}_valid_${ihr}_${level}mb_${var}_${stats}.png" ]; then
                    mv lead_average_regional_${domain}_valid_${ihr}_${level}mb_${var}_${stats}.png evs.global_ens.${evs_graphic_stats}.${var}_${plevel}.last${past_days}days.fhrmean_valid${ihr}_f384.g003_${domain_new}.png
                fi
                
		for lead in 24 120 240 360; do
                    lead_new=$(printf "%03d" "${lead}")
                    if [ -f "time_series_regional_${domain}_valid_${ihr}_${level}mb_${var}_${stats}_f${lead}.png" ]; then
                        mv time_series_regional_${domain}_valid_${ihr}_${level}mb_${var}_${stats}_f${lead}.png evs.global_ens.${evs_graphic_stats}.${var}_${plevel}.last${past_days}days.timeseries_valid${ihr}_f${lead_new}.g003_${domain_new}.png
                    fi
	        done # lead
            done # level
        done # var
