#!/bin/ksh
#*******************************************************************************
# Purpose: setup environment, paths, and run the sref cape ploting python script
...
if [ $SENDCOM = YES ] ; then
 cpreq  evs.plots.sref.cape.past${past_days}days.v${VDATE}.tar  $COMOUT/$STEP/$COMPONENT/$RUN.$VDATE/.  
fi
