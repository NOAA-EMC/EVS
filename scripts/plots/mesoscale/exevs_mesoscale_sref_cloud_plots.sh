...
if [ $SENDCOM="YES" ]; then
 cpreq  evs.plots.sref.cloud.past${past_days}days.v${VDATE}.tar  $COMOUT/$RUN.$VDATE/.  
fi

if [ $SENDDBN = YES ] ; then
     $DBNROOT/bin/dbn_alert MODEL EVS_RZDM $job $COMOUT/$RUN.$VDATE/evs.plots.sref.cloud.past${past_days}days.v${VDATE}.tar
fi
