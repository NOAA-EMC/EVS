#!/bin/ksh
#*******************************************************************************
# Purpose: setup environment, paths, and run the sref Td 2m ploting python script
...
if [ $SENDCOM="YES" ]; then
 cpreq  evs.plots.sref.td2m.past${past_days}days.v${VDATE}.tar  $COMOUT/$STEP/$COMPONENT/$RUN.$VDATE/.  
fi
