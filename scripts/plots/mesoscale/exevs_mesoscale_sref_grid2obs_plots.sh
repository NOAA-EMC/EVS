  ...
  for score_type in $score_types ; do
   ...
   for lead in $fcst_leads ; do 
	...
	if [ $VAR = UGRD ] || [ $VAR = VGRD ] || [ $VAR = HGT ] || [ $VAR = TMP ] ; then
	 ...
	 echo "     cp ${plot_dir}/${score_type}_regional_conus_valid_${vld}z_${level_var}_ets*.png $restart" >> run_${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.${fcst_valid_hour}.sh
	 echo "     cp ${plot_dir}/${score_type}_regional_conus_valid_${vld}z_${level_var}_fbias*.png $restart" >> run_${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.${fcst_valid_hour}.sh
	 echo "  else " >> run_${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.${fcst_valid_hour}.sh
	 echo "    cp ${plot_dir}/${score_type}_regional_conus_valid_${vld}z_${level_var}_${stats}*.png $restart" >> run_${stats}.${score_type}.${lead}.${VAR}.${FCST_LEVEL_value}.${line_type}.${fcst_valid_hour}.sh
...
if [ -s *.png ] ; then
 tar -cvf evs.plots.sref.grid2obs.last${last_days}days.v${VDATE}.tar *.png
fi
