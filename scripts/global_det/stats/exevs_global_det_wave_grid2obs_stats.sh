...
for theDate in ${date_list}; do
  cp ${ARCgfs}/${RUN}.${theDate}/${MODELNAME}/${VERIF_CASE}/*global.0p25*grib2 ${DATA}/gribs/.
done
...
then
  set +x
  echo "Successfully copied the GFS-Wave grib2 files for ${VDATE}"
  [[ "$LOUD" = YES ]] && set -x
else
  set +x
  echo ' '
  ...
  echo '**************************************** '
  echo ' '
  echo "${MODELNAME}_${RUN} $VDATE $cycle : GFS-Wave grib2 files missing."
  [[ "$LOUD" = YES ]] && set -x
  ./postmsg "$jlogfile" "FATAL ERROR : NO GFS-Wave GRIB2 FILES for ${VDATE}"
  err_exit "FATAL ERROR: Did not copy the GFS-Wave grib2 files for ${VDATE}"
fi
