...
# Create Job Script 
python $USHevs/mesoscale/mesoscale_plots_headline_create_job_scripts.py
status=$?
[[ $status -ne 0 ]] && exit $status
[[ $status -eq 0 ]] && echo "Successfully ran mesoscale_plots_headline_create_job_scripts.py"
export njob=$((njob+1))
...
if [ $USE_CFP = YES ]; then
    ...
    status=$?
    [[ $status -ne 0 ]] && exit $status
    [[ $status -eq 0 ]] && echo "Successfully ran mesoscale_plots_headline_create_poe_job_scripts.py"
fi
