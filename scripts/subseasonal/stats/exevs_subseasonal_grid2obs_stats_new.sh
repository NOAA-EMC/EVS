...
for group in reformat_data assemble_data generate_stats gather_stats; do
	...
	for WEEK in $WEEKLY_LIST; do
	    ...
	    python $USHevs/subseasonal/subseasonal_stats_grid2obs_create_weekly_reformat_job_scripts_new.py
	    status=$?
	    [[ $status -ne 0 ]] && exit $status
	...
	for DAYS in $DAYS6_10_LIST; do
	    ...
	    python $USHevs/subseasonal/subseasonal_stats_grid2obs_create_days6_10_reformat_job_scripts_new.py
	    status=$?
	    [[ $status -ne 0 ]] && exit $status
	...
	for WEEKS in $WEEKS3_4_LIST; do
	    ...
	    python $USHevs/subseasonal/subseasonal_stats_grid2obs_create_weeks3_4_reformat_job_scripts_new.py
	    status=$?
	    [[ $status -ne 0 ]] && exit $status
	...
	for WEEK in $WEEKLY_LIST; do
	    ...
	    python $USHevs/subseasonal/subseasonal_stats_grid2obs_create_weekly_assemble_job_scripts_new.py
	    status=$?
	    [[ $status -ne 0 ]] && exit $status
	...
	for DAYS in $DAYS6_10_LIST; do
	    ...
	    python $USHevs/subseasonal/subseasonal_stats_grid2obs_create_days6_10_assemble_job_scripts_new.py
	    status=$?
	    [[ $status -ne 0 ]] && exit $status
	...
	for WEEKS in $WEEKS3_4_LIST; do
	    ...
	    python $USHevs/subseasonal/subseasonal_stats_grid2obs_create_weeks3_4_assemble_job_scripts_new.py
	    status=$?
	    [[ $status -ne 0 ]] && exit $status
	...
	if [ $USE_CFP = YES ]; then
        ...
        echo "Creating and running jobs for grid-to-obs stats: ${JOB_GROUP}"
        python $USHevs/subseasonal/subseasonal_stats_grid2obs_create_job_scripts_new.py
        status=$?
        [[ $status -ne 0 ]] && exit $status
