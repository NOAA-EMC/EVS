#!/bin/sh
# Program Name: evs_cfs_subseasonal_stats
# Author(s)/Contact(s): Shannon Shields
# Abstract: Run METplus for subseasonal grid-to-grid and grid-to-obs verification to produce stats
# History Log:
...
[[ $status -eq 0 ]] && echo "Successfully ran check_subseasonal_config.py"
echo

# Set up environment variables for initialization, valid, and forecast hours and source them
python $USHevs/subseasonal/set_init_valid_fhr_subseasonal_info.py
status=$?
[[ $status -ne 0 ]] && exit $status
[[ $status -eq 0 ]] && echo "Successfully ran set_init_valid_fhr_sub_info.py"
echo
...
#mkdir -p data
#python $USHevs/subseasonal/get_cfs_subseasonal_data_files.py
#status=$?
#[[ $status -ne 0 ]] && exit $status
#[[ $status -eq 0 ]] && echo "Successfully ran get_cfs_subseason_data_files.py"
#echo
...
python $USHevs/subseasonal/create_METplus_subseasonal_job_scripts.py
status=$?
[[ $status -ne 0 ]] && exit $status
...
if [ $SENDMETVIEWER = YES ]; then
    python $USHevs/subseasonal/load_to_METviewer_AWS.py
    status=$?
    [[ $status -ne 0 ]] && exit $status
