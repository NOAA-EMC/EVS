...
periods='LAST30DYS LAST90DAYS'
small_periods=$(echo ${periods} | tr '[A-Z]' '[a-z]')
		...
		for wvar in ${wave_vars} ; do
			for stats in ${stats_list}; do
				for fhr in ${fhrs} ; do
					...
					if [ ptype == 'timeseries' ]; then
						imagename=evs.${COMPONENT}.${image_stat}.${wvar}_${image_level}_${obstype}.${small_period}.${ptype}_valid${vhr}z_f${fhr}.${region}.png
            					tmp_image=$DATA/ndbc_standard/$imagename
						...
						imagename=evs.${COMPONENT}.${image_stat}.${wvar}_${image_level}_${obstype}.${small_period}.fhrmean_valid${vhr}z_f144.${region}.png
            					tmp_image=$DATA/wave/$imagename
