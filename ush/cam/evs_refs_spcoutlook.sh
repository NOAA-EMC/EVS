...
mask_day3=${EVSINspcotlk:0:$index}/cam/$RUN.$day1/spc_otlk
   ...
   for valid in 00 12 ; do
    ...
    for fhr in 06 12 18 24 30 36 42 48 54 60 ; do
     ...
     if [ ! -e  $COMOUTrestart/spcoutlook/run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.completed ] ; then
       ...
       echo  "#!/bin/ksh" >>  run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh	     
       echo  "set -x" >>  run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh	     
       ...
       echo  "err=$?; err_chk" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "if [ \$? = 0 ] ; then" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "  echo completed >\$output_base/stat/\${MODEL}/run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.completed" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "  cp \$output_base/stat/\${MODEL}/*.stat $all_stats" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       #Copy  files to COMOUT directory
       echo "  if [ $SENDCOM = YES ] ; then" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "    mkdir -p $COMOUTsmall/run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "    cp -v \$output_base/stat/\${MODEL}/*.stat $COMOUTsmall/run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "    cp -v \$output_base/stat/\${MODEL}/run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.completed $COMOUTrestart/spcoutlook" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "  fi" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "fi" >> run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh

       chmod +x run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh
       echo "${DATA}/scripts/run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook.sh" >> run_all_refs_spcoutlook_poe.sh

      else
	#Copy stat files for restart
        if [ -s $COMOUTsmall/run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook/*.stat ] ; then
	  cp $COMOUTsmall/run_refs_${model}.${dom}.${valid}.${fhr}_spcoutlook/*.stat $all_stats 
	fi
      fi
