#!/usr/bin/env python3
'''
Name: cam_rrfs_chem_plots_grid2obs_create_job_scripts.py
...
base_plot_jobs_info_dict = {
    ...
    'pm10': {
        'PM10': {'vx_masks': ['CONUS', 'CONUS_Central', 'CONUS_East',
                              'CONUS_South', 'CONUS_West',
                              'Appalachia', 'CPlains', 'DeepSouth',
                              'GreatBasin', 'GreatLakes', 'Mezquital',
                              'MidAtlantic', 'NorthAtlantic',
...
for pm25_job in list(filter_stats_jobs_dict['pm25'].keys()):
    ...
    if pm25_job in ['PM25']:
        ## Use same setting as pm25_job, but, overwrite predefined variables for "Thresh" type
...
for pm10_job in list(filter_stats_jobs_dict['pm10'].keys()):
    filter_stats_jobs_dict['pm10'][pm10_job]['grid'] = 'NA'
    ...
    if pm10_job in ['PM10']:
        ## Use same setting as pm10_job, but, overwrite predefined variables for "Thresh" type
...
for aod_job in list(filter_stats_jobs_dict['aod'].keys()):
    ...
    if aod_job in ['AOD']:
        ## Use same setting as aod_job, but, overwrite predefined variables for "Thresh" type
