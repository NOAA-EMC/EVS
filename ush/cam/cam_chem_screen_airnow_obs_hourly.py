...
evs_model=os.environ['MODELNAME']
#
...
for line in rfile:
    if not flag_data:
        ...
        if num_var == num_ref_hdr:
            ...
            bad_rec += 1
if wcount == 0:
    print(f"WARNING: {input_file} is corrupt (wrong number of columns). {evs_model} {evs_run} {evs_step} step will skip the corrupted validation file.")
else:
    if bad_rec > 0:
