   ...
   if [ $dom = CONUS ] ; then
    ...
    if [ ! -e  $COMOUTrestart/profile/run_refs_${domain}.${valid_at}.${fhr}_profile.completed ] ; then      
      ...
      if [ -s $input_fcst ] && [ -s $input_obsv ] ; then      
       ...
       echo  "export verif_poly='${maskpath}/Bukovsky_G227_CONUS.nc'" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
        ...
        echo "  export err=\$?; err_chk" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh	
	...
	echo "  export err=\$?; err_chk" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
        echo "  if [ \$? = 0 ] ; then " >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
        echo "    echo completed >\$output_base/stat/\${MODEL}/run_refs_${domain}.${valid_at}.${fhr}_profile.EnsembleStat.completed" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "    cp \$output_base/stat/\${MODEL}/ensemble_stat*.stat $all_stats" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
        echo "    if [ $SENDCOM = YES ] ; then" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "      mkdir -p $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "      cp \$output_base/stat/\${MODEL}/ensemble_stat*.stat $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "      cp \$output_base/stat/\${MODEL}/run_refs_${domain}.${valid_at}.${fhr}_profile.EnsembleStat.completed $COMOUTrestart/profile" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "    fi" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "  fi" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "else " >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo " cp $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile/ensemble_stat*.stat $all_stats" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "fi" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	...
	echo "  ${METPLUS_PATH}/ush/run_metplus.py -c ${PARMevs}/metplus_config/machine.conf -c ${GRID2OBS_CONF}/PointStat_fcstREFS_obsPREPBUFR_PROFILE_prob.conf " >>  run_refs_${domain}.${valid_at}.${fhr}_profile.sh
        echo "  export err=\$?; err_chk" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "  if [ \$? = 0 ] ; then" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
        echo "    echo completed >\$output_base/stat/\${MODEL}/run_refs_${domain}.${valid_at}.${fhr}_profile.PointStat.completed" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "    cp \$output_base/stat/\${MODEL}/point_stat*.stat $all_stats" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "    if [ $SENDCOM = YES ] ; then" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "      [[ ! -d $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile ]] && mkdir -p $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "      cp \$output_base/stat/\${MODEL}/point_stat*.stat $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "      cp \$output_base/stat/\${MODEL}/run_refs_${domain}.${valid_at}.${fhr}_profile.PointStat.completed $COMOUTrestart/profile" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "    fi" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "  fi" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "else " >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo " cp $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile/point_stat*.stat $all_stats" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo "fi" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh

	echo "[[ \$? = 0 ]] && echo completed >\$output_base/stat/\${MODEL}/run_refs_${domain}.${valid_at}.${fhr}_profile.completed" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh

	echo "if [ $SENDCOM = YES ] ; then " >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh
	echo " cp \$output_base/stat/\${MODEL}/run_refs_${domain}.${valid_at}.${fhr}_profile.completed $COMOUTrestart/profile" >>  run_refs_${domain}.${valid_at}.${fhr}_profile.sh
        echo "fi" >> run_refs_${domain}.${valid_at}.${fhr}_profile.sh

       chmod +x run_refs_${domain}.${valid_at}.${fhr}_profile.sh
       echo "${DATA}/scripts/run_refs_${domain}.${valid_at}.${fhr}_profile.sh" >> run_all_refs_profile_poe.sh

       fi

      else
	if [ -s $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile/*.stat ] ; then 
	  cp $COMOUTsmall/run_refs_${domain}.${valid_at}.${fhr}_profile/*.stat $all_stats
        fi	  
      fi
