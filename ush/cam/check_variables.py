...
def check_IMG_HEADER(IMG_HEADER):
    if not isinstance(IMG_HEADER, str):
        print(f"FATAL ERROR: The provided IMG_HEADER ('{IMG_HEADER}') is not a string."
                     + f"  IMG_HEADER must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if re.search(r'[^A-Za-z0-9_\-\\.]', IMG_HEADER):
        print(f"FATAL ERROR: The provided IMG_HEADER string ('{IMG_HEADER}') contains"
...
def check_RESTART_DIR(RESTART_DIR):
    if not isinstance(RESTART_DIR, str):
        print(f"FATAL ERROR: The provided RESTART_DIR ('{RESTART_DIR}') is not a string."
                     + f"  RESTART_DIR must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not Path(RESTART_DIR).exists():
        print(f"WARNING: The provided RESTART_DIR ('{RESTART_DIR}') does not exist on the"
                       + f" current system.")
...
def check_STAT_OUTPUT_BASE_DIR(STAT_OUTPUT_BASE_DIR):
    if not isinstance(STAT_OUTPUT_BASE_DIR, str):
        print(f"FATAL ERROR: The provided STAT_OUTPUT_BASE_DIR ('{STAT_OUTPUT_BASE_DIR}') is not a string."
...
def check_LOG_TEMPLATE(LOG_TEMPLATE):
    if not isinstance(LOG_TEMPLATE, str):
        print(f"FATAL ERROR: The provided LOG_TEMPLATE ('{LOG_TEMPLATE}') is not a string."
...
def check_MODELS(MODELS):
    if not isinstance(MODELS, str):
        print(f"FATAL ERROR: The provided MODELS ('{MODELS}') is not a string."
                     + f" MODELS must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not re.search(r'(^[ A-Za-z0-9,\-_?=]+)$', MODELS):
        print(
            f"FATAL ERROR: The provided MODELS ('{MODELS}') is not valid. "
            + "MODELS may contain letters, numbers, hyphens, underscores, "
            + "commas, spaces, and supported URL-style query parameters. "
            + "Check the plotting configuration file."
        )
        sys.exit(1)
    return MODELS
