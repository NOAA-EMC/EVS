    ...
    for valid_run in 00 03 06 09 12 15 18 21 ; do
     ...
     if [ ! -e  $COMOUTrestart/product/run_refs_${model}.${dom}.${valid_run}.${fhr}_product.completed ] ; then
      ...
      if [ -s $input_fcst ] && [ -s $input_obsv ] ; then
       ...
       echo  "set -x" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh       
       ...
       echo  "export err=\$?; err_chk" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh 

       echo " if [ \$? = 0 ] ; then" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "  echo completed >\$output_base/stat/\${MODEL}/run_refs_${model}.${dom}.${valid_run}.${fhr}_product.completed" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "  cp -v \$output_base/stat/\${MODEL}/*.stat $all_stats" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh

       echo  " if [ $SENDCOM = YES ] ; then " >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "    mkdir -p $COMOUTsmall/run_refs_${model}.${dom}.${valid_run}.${fhr}.product" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo  "   cp -v \$output_base/stat/\${MODEL}/*.stat  $COMOUTsmall/run_refs_${model}.${dom}.${valid_run}.${fhr}.product" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       #Mark this CONUS task is completed
       echo "    cp -v \$output_base/stat/\${MODEL}/run_refs_${model}.${dom}.${valid_run}.${fhr}_product.completed $COMOUTrestart/product" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "  fi" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "fi" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh

       chmod +x run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "${DATA}/scripts/run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh" >> run_all_refs_product_poe.sh

      fi

     else
       #Copy stat files from restart
       if [ -s $COMOUTsmall/run_refs_${model}.${dom}.${valid_run}.${fhr}.product/*.stat ] ; then
	    cp $COMOUTsmall/run_refs_${model}.${dom}.${valid_run}.${fhr}.product/*.stat $all_stats
       fi
     fi 
    ...
    for fhr in $fhrs ; do
     ...
     if [ ! -e  $COMOUTrestart/product/run_refs_${model}.${dom}.${valid_run}.${fhr}_product.completed ] ; then
      ...
      if [ -s $input_fcst ] && [ -s $input_obsv ] ; then
       ...
       echo " if [ \$? = 0 ] ; then" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "  echo completed >\$output_base/stat/\${MODEL}/run_refs_${model}.${dom}.${valid_run}.${fhr}_product.completed" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "  cp \$output_base/stat/\${MODEL}/*.stat $all_stats" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "  if [ $SENDCOM = YES ] ; then " >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "    mkdir -p $COMOUTsmall/run_refs_${model}.${dom}.${valid_run}.${fhr}.product" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "    cp -v \$output_base/stat/\${MODEL}/*.stat  $COMOUTsmall/run_refs_${model}.${dom}.${valid_run}.${fhr}.product" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
       echo "    cp -v \$output_base/stat/\${MODEL}/run_refs_${model}.${dom}.${valid_run}.${fhr}_product.completed $COMOUTrestart/product" >> run_refs_${model}.${dom}.${valid_run}.${fhr}.product.sh
