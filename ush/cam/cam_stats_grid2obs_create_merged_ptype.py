...
if len(sys.argv) != 1:
    print("ERROR: Not given correct number of run time arguments..."
          ...
          +"FCST_INFILE")
    sys.exit(1)
input_grib2_file_format = sys.argv[1]
...
input_grib2_file = cam_util.format_filler(
    input_grib2_file_format, valid_date_dt, init_date_dt, str(fhr), {}
)
...
temp_dir = os.path.join(DATA, VERIF_CASE, 'data', MODELNAME, 'tmp')
temp_fname = (f'{NET}_{COMPONENT}_{RUN}_{STEP}_{VERIF_CASE}_{VERIF_TYPE}'
              + f'_{MODELNAME}_{job_name}_v{VDATE}{VHOUR}_f{fhr}.nc')
input_nc_file = os.path.join(temp_dir, temp_fname)

if os.path.exists(input_wgrib2_file):
    cutil.run_shell_command([
        'wgrib2', '-match', '\":(CRAIN|CSNOW|CFRZR|CICEP):\"', 
        input_wgrib2_file, '-netcdf', input_nc_file
    ])
    if os.path.exists(input_nc_file:
        ...
        for ptype in ptypes:
            if not ptype in input_data.variables:
                print("WARNING: "+ptype+" does not exist in "+input_wgrib2_file)
                all_input_ptype_files_exist = False
        if all_input_ptype_files_exist:
            print("\nInput File: "+input_nc_file)
    
            input_crain = input_data.variables['CRAIN_surface'][:][0]
            input_csnow = input_data.variables['CSNOW_surface'][:][0]
            input_cfrzr = input_data.variables['CFRZR_surface'][:][0]
            input_cicep = input_data.variables['CICEP_surface'][:][0]
