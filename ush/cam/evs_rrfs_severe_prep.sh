...
i=1
   ...
   while [ $i -le $min_file_req ]; do
      ...
      if [ $MEMNUM = ctl ]; then
         ...
         export fcst_file=refs.${INITDATE}/${vhr}/mem000${MEMNUM}/${MODELNAME}.t${vhr}z.m0${MEMNUM}.prslev.3km.f$(printf "%03d" $fhr).conus.grib2
      fi
   ...
   if [ $nfiles -eq $min_file_req ]; then
      ...
      if [ $SENDMAIL = YES ]; then
          ...
          echo "Warning: Only $nfiles ${MODELNAME} mem000${MEMNUM} forecast files found for ${vhr}Z ${INITDATE} cycle. $min_file_req files are required. METplus will not run." > mailmsg${JOBNUM}
