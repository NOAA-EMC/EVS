        ...
        with open(log_missing_file, "w") as lmf:
            ...
            if fhr == 'anl':
                lmf.write(f'export subject="{model.upper()} Analysis '
                          +'Data Missing for EVS rrfs-chem"\n')
...
def check_plot_files(job_dict):
    ...
    if job_dict['JOB_GROUP'] in ['filter_stats', 'make_plots']:
        ...
        fdays = list(
            range(int(job_dict['fday_start']),
                  int(job_dict['fday_end'])+int(job_dict['fday_inc']),
                  int(job_dict['fday_inc']))
        )
    if job_dict['JOB_GROUP'] == 'make_plots':
        from cam_rrfs_chem_plots_specs import PlotSpecs
...
def get_daily_stat_file(model_name, source_stats_base_dir,
    ...
    """! Link model daily stat files
         Args:
             ...
             source_stats_base_dir     - full path to stats/cam
...
def build_df_fhr_mean(job_group, logger, input_dir, output_dir, model_info_dict,
    ...
    if filter_inithr_flag:
        filter_filename_label=f"fcstday{selected_fcst_day}_init{filter_init_hour}z"
    else:
        filter_filename_label=f"fcstday{selected_fcst_day}"
    for model_num in list(model_info_dict.keys()):
        model_num_name = (
            model_num+'/'+model_info_dict[model_num]['name']
            +'/'+model_info_dict[model_num]['plot_name']
        ...
        if len(dates) != 0:
            filtered_model_stat_file_name = (
                ...
                +'grid'+grid+'_'+'vxmask'+vx_mask+'_'
                +'interp'+interp_method+interp_points+'_'
                +filter_filename_label.lower()+"_"+date_type.lower()
            ).lower().replace('.','p').replace('-', '_')\
            .replace('&&', 'and').replace('||', 'or')\
            .replace('0,*,*', '').replace('*,*', '')+'.stat'
            input_filtered_model_stat_file = os.path.join(
        ...
        if write_filtered_stat_file:
            ...
            if os.path.exists(condensed_model_file):
                ...
                fcst_lead_hours = filtered_model_df['FCST_LEAD'].str[:2].astype(int)
                valid_datetimes = pd.to_datetime(filtered_model_df['FCST_VALID_BEG'], format='%Y%m%d_%H%M%S')
                selected_forecast_hours = [int(fhr) for fhr in forecast_hours]

                init_hours = (valid_datetimes.dt.hour - (fcst_lead_hours % 24)) % 24

                mask = filtered_model_df['FCST_VALID_BEG'].isin(met_format_valid_dates)

                filter_inithr_flag=False
                filter_inithr_flag=True
                if filter_inithr_flag:
                    ## logger.debug("Applying conditional filter for INIT_HOUR...")
                    mask &= (init_hours == int(filter_init_hour))

                if filter_fhr_flag:
                    logger.debug(f"selected_forecast_hours={selected_forecast_hours}")

                    ## logger.debug("Applying conditional filter for FCST_LEAD...")
                    mask &= fcst_lead_hours.isin(selected_forecast_hours)

                # Apply the final, combined filter to the DataFrame
                final_df = filtered_model_df[mask].copy()

                # Sort and save the final DataFrame to CSV
                final_df = final_df.sort_values(by='FCST_VALID_BEG')

                final_df.to_csv(
                    filtered_model_stat_file,
                    header=met_version_line_type_col_list,
                    index=None,
                    sep=' ',
                    mode='w'
                )
