  ...
  for prod in mean pmmn avrg lpmm prob eas system ; do
    ...
    for acc in $acum  ; do
	 ...
	 for vhr in $vhrs; do
		 ...
		 input_fcst=${modelpath}/href.${iday}/verf_g2g/href.m??.t${ihr}z.${domain}.f${fhr}
	      elif [ $extra = "ensprod" ] ; then
		 input_fcst=${modelpath}/href.${iday}/ensprod/href.t${ihr}z.${domain}.${prod}.f${fhr}.grib2
              else
		 input_fcst=${modelpath}/href.${iday}/href${prod}.t${ihr}z.${grid}.24h.f${fhr}.nc
	      fi
	      ...
	      input_obsv="$WORK/${obsvtype}.${VDATE}/${obsv}.t${vhr}z.*"
	          ...
	          if  [ $prod = system ] && [ $domain = conus ] ; then
		  ...
		  elif [ $prod = system ] && [ $domain = ak ] ; then
