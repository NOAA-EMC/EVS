#!/usr/bin/env python3
'''
Name: cam_rrfs_chem_plots_time_series_fhr_mean.py
...
class TimeSeriesFhrMean:
    ...
    def make_time_series_fhr_mean(self):
        ...
        fhr_max = self.date_info_dict.get('fhr_max')

        # This checks if fhr_max is exactly the Python None object
        if fhr_max is None:
        ...
        if len(fcst_units) > 1:
            ...
            fcst_units = ['']
        plot_title = plot_specs_ts.get_plot_title_rrfs_chem(
...
def main():
    ...
    p = TimeSeriesFhrMean(logger, INPUT_DIR, OUTPUT_DIR, MODEL_INFO_DICT,
                   DATE_INFO_DICT, PLOT_INFO_DICT, MET_INFO_DICT, LOGO_DIR)
    p.make_time_series_fhr_mean()
