...
>run_gather_${verify}.sh
    ...
    if [ $SENDCOM = 'YES' ] && [ -s ${WORK}/gather/${vday}/${modnam}_${verify}_${vday}.stat ] ; then
   echo "cp -v ${WORK}/gather/${vday}/${modnam}_${verify}_${vday}.stat  $COMOUTfinal/evs.stats.${model}.${verify}.v${vday}.stat">>run_gather_${verify}.sh
    fi
