#!/usr/bin/env python3
'''
Name: global_chem_atmnos_plots_grid2obs_create_job_scripts.py
        ...
        for loop_info in JOB_GROUP_verif_type_job_product_loops:
            if JOB_GROUP in ['condense_stats', 'filter_stats']:
                ...
                if write_job_cmds:
                    ...
                    job.write(
                        gda_util.python_command('global_chem_atmnos_plots.py',[])
                        +'\n'
                    )
                ...
                for plot_loop_info in list(
                    ...
                    run_global_chem_atmnos_plots = ['plots']
                    if evs_run_mode == 'production' and \
                            verif_type in ['aeronet', 'airnow'] and \
                            job_env_dict['plot'] in \
                            ['lead_average']:
                        run_global_chem_atmnos_plots.append('plots_tof120')
                    for run_global_chem_atmnos_plot in run_global_chem_atmnos_plots:
                        # Set up output directories
                        njobs+=1
                        job_env_dict['job_id'] = 'job'+str(njobs)
                        job_work_dir, job_DATA_dir, job_COMOUT_dir = (
                        ...
                        for name, value in job_env_dict.items():
                            if name not in dont_write_env_var_list:
                                job.write('export '+name+'="'+value+'"\n')
                        job.write('\n')
                        if run_global_chem_atmnos_plot == 'plots_tof120':
                            fhrs_tof120 = []
                            for fhr in job_env_dict['fhr_list'].split(', '):
                                if int(fhr) <= 120:
