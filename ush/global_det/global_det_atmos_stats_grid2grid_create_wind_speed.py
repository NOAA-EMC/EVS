#!/usr/bin/env python3
'''
Name: global_det_atmos_stats_grid2grid_create_wind_speed.py
...
while valid_date_dt <= ENDDATE_dt:
    ...
    for fhr_str in [f for f in fhr_list if f.strip()]:
        ...
        output_file = os.path.join(
            full_path_job_num_work_dir, f"wind_speed_{VERIF_TYPE}_{job_name}_"
        ...
        if os.path.exists(output_file_COMOUT):
            ...
            make_wind_speed_output_file = False
            gda_util.copy_file(output_file_COMOUT, output_file_DATA)
        else:
            make_wind_speed_output_file = True
        if found_input and make_wind_speed_output_file:
               ...
               make_wind_speed_output_file = True
                ...
                for req_var_level in req_var_level_list:
                    if req_var_level not in input_file_data_var_list:
                        print("NOTE: Cannot make wind speed file "
            ...
            print("NOTE: Cannot make wind speed file "
                  ...
                  +"do not exist")
            make_wind_speed_output_file = False
        if make_wind_speed_output_file:
            ...
            for data_name in ['FCST', 'OBS']:
                ...
                data_name_ugrd250 = (
                    input_file_data.variables[data_name+'_UGRD_P250_FULL'][:]
                )
                data_name_vgrd250 = (
                    input_file_data.variables[data_name+'_VGRD_P250_FULL'][:]
                )
                data_name_wind850 = np.sqrt(
                    data_name_ugrd850**2 + data_name_vgrd850**2
                )
                data_name_wind250 = np.sqrt(
                    data_name_ugrd250**2 + data_name_vgrd250**2
                )
                wind_data_arrays = [data_name_wind850, data_name_wind250]
                for level, wind_data in zip(levels, wind_data_arrays):
                    write_data_name_var = output_file_data.createVariable(
                        data_name+'_WSPD_'+level,
                        input_file_data.variables[data_name+'_UGRD_P850_FULL']\
                        .datatype,
                        input_file_data.variables[data_name+'_UGRD_P850_FULL']\
                        .dimensions
                    ...
                    for k \
                        ...
                        if k == 'name':
                            write_data_name_var.setncatts(
                                {k: data_name+'_WSPD_'+level}
                            )
                        elif k == 'long_name':
                            write_data_name_var.setncatts(
                                {k: 'Wind Speed for '+level}
                            )
                        elif k == 'level':
                            write_data_name_var.setncatts(
                                {k: level}
