#!/usr/bin/env python3
'''
Name: global_det_atmos_stats_wmo_format_rec2_monthly.py
...
# Format for WMO monthly rec2
## grid2grid_upperair
grid2grid_upperair_VDATE_monthly_rec2_lines = []
wmo_ref = 'an'
...
for wmo_t in wmo_t_list:
    stat_file = os.path.join(
        ...
        f"{MODELNAME}.grid2grid_upperair.{VDATE_dt:%Y%m}_{wmo_t.zfill(2)}Z"
        +'.summary.stat'
    )
    ...
    for met_line_type in list(stat_dict.keys()):
        met_iter_list = list(
        ...
        for met_iter in met_iter_list:
            ...
            if wmo_sc == 'sd' and str(wmo_s) == '12':
                if have_ostdev_stat_line:
                    ...
                    wmo_v_sd_0 = 'nil'
                grid2grid_upperair_VDATE_monthly_rec2_lines.append(
...
with open(tmp_VDATE_monthly_rec2_file, 'w') as f:
    for line in grid2grid_upperair_VDATE_monthly_rec2_lines:
        f.write(line)
