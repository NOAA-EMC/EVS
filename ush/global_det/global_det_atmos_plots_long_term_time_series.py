'''
Name: global_det_atmos_plots_long_term_time_series.py
...
class LongTermTimeSeries:
    """
    Create long term time series plot
    """
    ...
    def __init__(self, logger, input_dir, output_dir, logo_dir,
        ...
        """! Initalize LongTermTimeSeries class
             Args:
    ...
    def make_long_term_time_series(self):
        """! Create the long term time series graphic
             Args:
             Returns:
        """
        ...
        for run_length in self.run_length_list:
            ...
            plot_specs_ltts = PlotSpecs(self.logger,
                                         'long_term_time_series')
            plot_specs_ltts.set_up_plot()
            left_logo_xpixel_loc, left_logo_ypixel_loc, left_logo_alpha = (
                plot_specs_ltts.get_logo_location(
                    'left', plot_specs_ltts.fig_size[0],
                    plot_specs_ltts.fig_size[1], plt.rcParams['figure.dpi']
                )
            )
            right_logo_xpixel_loc, right_logo_ypixel_loc, right_logo_alpha = (
                plot_specs_ltts.get_logo_location(
                    'right', plot_specs_ltts.fig_size[0],
                     plot_specs_ltts.fig_size[1], plt.rcParams['figure.dpi']
                )
            )
            all_model_plot_settings_dict = (
                plot_specs_ltts.get_model_plot_settings()
            )
            for forecast_day in self.forecast_day_list:
                ...
                model_group_forecast_day = np.ma.masked_invalid(
                    run_length_model_group_merged_df[
                        ['DAY'+str(forecast_day)]
                    ].to_numpy(dtype=float).reshape(shape)
                )
                stat_min = model_group_forecast_day.min()
                stat_max = model_group_forecast_day.max()
                fig, ax = plt.subplots(
                    1,1,
                    figsize=(plot_specs_ltts.fig_size[0],
                             plot_specs_ltts.fig_size[1]),
                )
                if self.time_range == 'monthly':
                    dates_for_title = (
                ...
                fig.suptitle(
                    plot_specs_ltts.get_stat_plot_name(self.stat)+' - G004/'
                    +plot_specs_ltts.get_vx_mask_plot_name(self.vx_mask)+'\n'
                    +plot_specs_ltts.get_var_plot_name(self.var_name,
                                                       self.var_level)+" "
                    +f"({var_units})\n"
                ...
                ax.set_ylabel(plot_specs_ltts.get_stat_plot_name(self.stat))
                ax.set_xlabel('Year')
                if plot_left_logo:
                ...
                if len(ax.lines) != 0:
                    ...
                    y_axis_tick_inc = (ax.get_yticks()[1]
                                       - ax.get_yticks()[0])
                    legend = ax.legend(
                        bbox_to_anchor=(plot_specs_ltts.legend_bbox[0],
                                        plot_specs_ltts.legend_bbox[1]),
                        loc=plot_specs_ltts.legend_loc,
                        ncol=len(self.model_list),
                        fontsize=plot_specs_ltts.legend_font_size
                    )
                    plt.draw()
...
def main():
    ...
    p = LongTermTimeSeries(logger, INPUT_DIR, OUTPUT_DIR, LOGO_DIR,
                           TIME_RANGE, DATE_DT_LIST, MODEL_GROUP,
                           MODEL_LIST, VAR_NAME, VAR_LEVEL, VAR_THRESH,
                           VX_MASKK, STAT, FORECAST_DAY_LIST, RUN_LENGTH_LIST)
    p.make_long_term_time_series()
