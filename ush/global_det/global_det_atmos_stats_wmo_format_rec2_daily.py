#!/usr/bin/env python3
'''
Name: global_det_atmos_stats_wmo_format_rec2_daily.py
Contact(s): Mallory Row (mallory.row@noaa.gov)
Abstract: This gather the GridStat stat files and puts them in one
          file in the WMO required format
...
## grid2grid_upperair
grid2grid_upperair_VDATE_daily_rec2_lines = []
wmo_ref = 'an'
...
stat_file_dict = {
    'GRAD': {'stat_list': ['S1'],
             'var_list': ['PRMSL/Z0']},
    'CNT': {'stat_list': ['RMSE', 'ME', 'ANOM_CORR', 'MAE', 'RMSFA', 'RMSOA',
                          'FSTDEV'],
            'var_list': ['PRMSL/Z0',
                         'HGT/P850', 'HGT/P500', 'HGT/P250', 'HGT/P100',
                         'TMP/P850', 'TMP/P500', 'TMP/P250', 'TMP/P100',
                         'RH/P850', 'RH/P700']},
...
met_vx_mask_list = ['NHEM', 'SHEM', 'TROPICS', 'EURNAFR', 'NAMER', 'ASIA',
                    'AUSTNZ', 'NPOL', 'SPOL']
...
for stat_file_iter in stat_file_iter_list:
    ...
    stat_file = os.path.join(
        ...
        f"grid_stat_grid2grid_upperair_{met_line_type}_"
        ...
        for met_vx_mask in met_vx_mask_list:
            ...
            if have_stat_file:
                stat_line = stat_file_df[
                    ...
                    & (stat_file_df['FCST_VAR'] == met_var_level.split('/')[0])
                    & (stat_file_df['FCST_LEV'] == met_var_level.split('/')[1])
                    & (stat_file_df['OBS_VAR'] == met_var_level.split('/')[0])
                    & (stat_file_df['OBS_LEV'] == met_var_level.split('/')[1])
            ...
            for met_stat in stat_file_dict[met_line_type]['stat_list']:
                ...
                if have_stat_line:
                    wmo_v = str(round(float(stat_line.iloc[0][met_stat]),3))
                else:
                    wmo_v = 'nil'
...
with open(tmp_VDATE_daily_rec2_file, 'w') as f:
    if have_VDATEm1_daily_rec2:
        ...
        for line in VDATEm1_daily_rec2_lines:
            f.write(line)
    for line in grid2grid_upperair_VDATE_daily_rec2_lines:
        f.write(line)
