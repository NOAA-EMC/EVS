...
if [ $verify = upper ] ; then 
	...
	mbrs=1
	  ...
	  echo  "${METPLUS_PATH}/ush/run_metplus.py -c ${PARMevs}/metplus_config/machine.conf -c ${GRID2GRID_CONF}/GridStat_fcstGFS_obsModelAnalysis_climoERA5.conf " >> run_${modnam}_valid_at_t${vhour}z_${fhr}_g2g.sh
        ...
        [[ $SENDCOM="YES" ]] && echo "cp \$output_base/stat/${modnam}/grid_stat*.stat $COMOUTsmall" >> run_${modnam}_valid_at_t${vhour}z_${fhr}_g2g.sh
