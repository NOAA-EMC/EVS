...
def check_VERIF_CASE(VERIF_CASE):
    if not isinstance(VERIF_CASE, str):
        print("FATAL ERROR: The provided VERIF_CASE ('{VERIF_CASE}') is not a string."
                     + f"  VERIF_CASE must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not VERIF_CASE:
        print("FATAL ERROR: The provided VERIF_CASE is empty. VERIF_CASE cannot be"
...
def check_VERIF_TYPE(VERIF_TYPE):
    if not isinstance(VERIF_TYPE, str):
        print("FATAL ERROR: The provided VERIF_TYPE ('{VERIF_TYPE}') is not a string."
                     + f"  VERIF_TYPE must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not VERIF_TYPE:
        print("FATAL ERROR: The provided VERIF_TYPE is empty. VERIF_TYPE cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    return VERIF_TYPE
...
def check_URL_HEADER(URL_HEADER):
    if not isinstance(URL_HEADER, str):
        print("FATAL ERROR: The provided URL_HEADER ('{URL_HEADER}') is not a string."
                     + f"  URL_HEADER must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if re.search(r'[^A-Za-z0-9_\-\\]', URL_HEADER):
        print("FATAL ERROR: The provided URL_HEADER string ('{URL_HEADER}') contains"
...
def check_USH_DIR(USH_DIR):
    if not isinstance(USH_DIR, str):
        print("FATAL ERROR: The provided USH_DIR ('{USH_DIR}') is not a string."
...
def check_PRUNE_DIR(PRUNE_DIR):
    if not isinstance(PRUNE_DIR, str):
        print("FATAL ERROR: The provided PRUNE_DIR ('{PRUNE_DIR}') is not a string."
...
def check_SAVE_DIR(SAVE_DIR):
    if not isinstance(SAVE_DIR, str):
        print("FATAL ERROR: The provided SAVE_DIR ('{SAVE_DIR}') is not a string."
...
def check_FIX_DIR(FIX_DIR):
    if not isinstance(FIX_DIR, str):
        print("FATAL ERROR: The provided FIX_DIR ('{FIX_DIR}') is not a string."
...
def check_OUTPUT_BASE_DIR(OUTPUT_BASE_DIR):
    if not isinstance(OUTPUT_BASE_DIR, str):
        print("FATAL ERROR: The provided OUTPUT_BASE_DIR ('{OUTPUT_BASE_DIR}') is not a string."
...
def check_LOG_METPLUS(LOG_METPLUS):
    if not isinstance(LOG_METPLUS, str):
        print("FATAL ERROR: The provided LOG_METPLUS ('{LOG_METPLUS}') is not a string."
...
def check_LOG_LEVEL(LOG_LEVEL):
    if not isinstance(LOG_LEVEL, str):
        print("FATAL ERROR: The provided LOG_LEVEL ('{LOG_LEVEL}') is not a string."
    ...
    if not LOG_LEVEL:
        print("FATAL ERROR: The provided LOG_LEVEL is empty. LOG_LEVEL cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    return LOG_LEVEL
...
def check_MET_VERSION(MET_VERSION):
    if not isinstance(MET_VERSION, str):
        print("FATAL ERROR: The provided MET_VERSION ('{MET_VERSION}') is not a string."
                     + f" MET_VERSION must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not re.search(r'^[1-9]\d*(\.\d+)?$', MET_VERSION):
        print("FATAL ERROR: The provided MET_VERSION ('{MET_VERSION}') is not a"
...
def check_MODEL(MODEL):
    if not isinstance(MODEL, str):
        print("FATAL ERROR: The provided MODEL ('{MODEL}') is not a string."
                     + f" MODEL must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not re.search(r'(^[ A-Za-z0-9,\-_]+)$', MODEL):
        print("FATAL ERROR: The provided MODEL ('{MODEL}') is not valid. MODEL may"
...
def check_OBTYPE(OBTYPE):
    if not isinstance(OBTYPE, str):
        print("FATAL ERROR: The provided OBTYPE ('{OBTYPE}') is not a string."
                     + f" OBTYPE must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not re.search(r'(^[ A-Za-z0-9,\-_]+)$', OBTYPE):
        print("FATAL ERROR: The provided OBTYPE ('{OBTYPE}') is not valid. OBTYPE may"
                     + f" contain letters, numbers, hyphens, underscores,"
                     + f" commas, and spaces only. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
...
def check_RUN(RUN):
    if not isinstance(RUN, str):
        print("FATAL ERROR: The provided RUN ('{RUN}') is not a string."
                     + f" RUN must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not re.search(r'(^[ A-Za-z0-9,\-_]+)$', RUN):
        print("FATAL ERROR: The provided RUN ('{RUN}') is not valid. RUN may"
                     + f" contain letters, numbers, hyphens, underscores,"
                     + f" commas, and spaces only. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
...
def check_DATE_TYPE(DATE_TYPE):
    if not isinstance(DATE_TYPE, str):
        print("FATAL ERROR: The provided DATE_TYPE ('{DATE_TYPE}') is not a string."
                     + f" DATE_TYPE must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if str(DATE_TYPE).upper() not in ['INIT', 'VALID']:
        print("FATAL ERROR: You provided the following DATE_TYPE: '{DATE_TYPE}'."
...
def check_EVAL_PERIOD(EVAL_PERIOD):
    if not isinstance(EVAL_PERIOD, str):
        print("FATAL ERROR: The provided EVAL_PERIOD ('{EVAL_PERIOD}') is not a string."
                     + f" EVAL_PERIOD must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if re.search(r'[^A-Za-z0-9_\-\\]', EVAL_PERIOD):
        print("FATAL ERROR: The provided EVAL_PERIOD string ('{EVAL_PERIOD}') contains"
...
def check_VALID_BEG(VALID_BEG, DATE_TYPE, EVAL_PERIOD, plot_type=None):
    if not isinstance(VALID_BEG, str):
        print("FATAL ERROR: The provided VALID_BEG ('{VALID_BEG}') is not a string."
    ...
    if ((str(DATE_TYPE).upper() == "VALID" 
        ...
        if not VALID_BEG:
            print("FATAL ERROR: The provided VALID_BEG is empty. Since DATE_TYPE is"
        ...
        if not VALID_BEG.isdigit():
            print("FATAL ERROR: The provided VALID_BEG ('{VALID_BEG}') contains"
                         + f" non-numeric characters. VALID_BEG may only"
                         + f" contain numeric characters. Check the plotting"
                         + f" configuration file.")
            sys.exit(1)
        if not len(VALID_BEG) == 8:
            print("FATAL ERROR: The provided VALID_BEG ('{VALID_BEG}') is too short"
...
def check_VALID_END(VALID_END, DATE_TYPE, EVAL_PERIOD, plot_type=None):
    if not isinstance(VALID_END, str):
        print("FATAL ERROR: The provided VALID_END ('{VALID_END}') is not a string."
    ...
    if ((str(DATE_TYPE).upper() == "VALID" 
            ...
            and EVAL_PERIOD == "TEST"):
        if not VALID_END:
            print("FATAL ERROR: The provided VALID_END is empty. Since DATE_TYPE is"
                         + f" '{str(DATE_TYPE).upper()}', plot_type is"
                         + f" '{str(plot_type).lower()}', and EVAL_PERIOD is"
        ...
        if not VALID_END.isdigit():
            print("FATAL ERROR: The provided VALID_END ('{VALID_END}') contains"
                         + f" non-numeric characters. VALID_END may only"
                         + f" contain numeric characters. Check the plotting"
                         + f" configuration file.")
            sys.exit(1)
        if not len(VALID_END) == 8:
            print("FATAL ERROR: The provided VALID_END ('{VALID_END}') is too short"
...
def check_INIT_BEG(INIT_BEG, DATE_TYPE, EVAL_PERIOD, plot_type=None):
    if not isinstance(INIT_BEG, str):
        print("FATAL ERROR: The provided INIT_BEG ('{INIT_BEG}') is not a string."
    ...
    if ((str(DATE_TYPE).upper() == "INIT" 
            ...
            and EVAL_PERIOD == "TEST"):
        if not INIT_BEG:
            print("FATAL ERROR: The provided INIT_BEG is empty. Since DATE_TYPE is"
                         + f" '{str(DATE_TYPE).upper()}', plot_type is"
                         + f" '{str(plot_type).lower()}', and EVAL_PERIOD is"
        ...
        if not INIT_BEG.isdigit():
            print("FATAL ERROR: The provided INIT_BEG ('{INIT_BEG}') contains"
                         + f" non-numeric characters. INIT_BEG may only"
                         + f" contain numeric characters. Check the plotting"
                         + f" configuration file.")
            sys.exit(1)
        if not len(INIT_BEG) == 8:
            print("FATAL ERROR: The provided INIT_BEG ('{INIT_BEG}') is too short"
...
def check_INIT_END(INIT_END, DATE_TYPE, EVAL_PERIOD, plot_type=None):
    if not isinstance(INIT_END, str):
        print("FATAL ERROR: The provided INIT_END ('{INIT_END}') is not a string."
    ...
    if ((str(DATE_TYPE).upper() == "INIT" 
            ...
            and EVAL_PERIOD == "TEST"):
        if not INIT_END:
            print("FATAL ERROR: The provided INIT_END is empty. Since DATE_TYPE is"
                         + f" '{str(DATE_TYPE).upper()}', plot_type is"
                         + f" '{str(plot_type).lower()}', and EVAL_PERIOD is"
        ...
        if not INIT_END.isdigit():
            print("FATAL ERROR: The provided INIT_END ('{INIT_END}') contains"
                         + f" non-numeric characters. INIT_END may only"
                         + f" contain numeric characters. Check the plotting"
                         + f" configuration file.")
            sys.exit(1)
        if not len(INIT_END) == 8:
            print("FATAL ERROR: The provided INIT_END ('{INIT_END}') is too short"
...
def check_FCST_INIT_HOUR(FCST_INIT_HOUR, DATE_TYPE, plot_type=None):
    if not isinstance(FCST_INIT_HOUR, str):
        print("FATAL ERROR: The provided FCST_INIT_HOUR ('{FCST_INIT_HOUR}') is not a string."
    ...
    if (str(DATE_TYPE).upper() == "INIT" 
        ...
        if not FCST_INIT_HOUR:
            print("FATAL ERROR: The provided FCST_INIT_HOUR is empty. Since DATE_TYPE is"
        ...
        if not re.search(r'(^[ 0-9,]+)$', FCST_INIT_HOUR):
            print("FATAL ERROR: The provided FCST_INIT_HOUR ('{FCST_INIT_HOUR}') is"
...
def check_FCST_VALID_HOUR(FCST_VALID_HOUR, DATE_TYPE, plot_type=None):
    if not isinstance(FCST_VALID_HOUR, str):
        print("FATAL ERROR: The provided FCST_VALID_HOUR ('{FCST_VALID_HOUR}') is not a string."
    ...
    if (str(DATE_TYPE).upper() == "VALID" 
        ...
        if not FCST_VALID_HOUR:
            print("FATAL ERROR: The provided FCST_VALID_HOUR is empty. Since DATE_TYPE is"
                         + f" '{str(DATE_TYPE).upper()}' and plot type is"
                         + f" '{str(plot_type).lower()}', FCST_VALID_HOUR cannot be"
                         + f" empty. Check the plotting configuration file.")
            sys.exit(1)
        if not re.search(r'(^[ 0-9,]+)$', FCST_VALID_HOUR):
            print("FATAL ERROR: The provided FCST_VALID_HOUR ('{FCST_VALID_HOUR}') is"
...
def check_FCST_LEVEL(FCST_LEVEL):
    if not isinstance(FCST_LEVEL, str):
        print("FATAL ERROR: The provided FCST_LEVEL ('{FCST_LEVEL}') is not a string."
                     + f" FCST_LEVEL must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not FCST_LEVEL:
        print("FATAL ERROR: The provided FCST_LEVEL is empty. FCST_LEVEL cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    return FCST_LEVEL
...
def check_OBS_LEVEL(OBS_LEVEL):
    if not isinstance(OBS_LEVEL, str):
        print("FATAL ERROR: The provided OBS_LEVEL ('{OBS_LEVEL}') is not a string."
                     + f" OBS_LEVEL must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not OBS_LEVEL:
        print("FATAL ERROR: The provided OBS_LEVEL is empty. OBS_LEVEL cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    return OBS_LEVEL
...
def check_var_name(var_name):
    if not isinstance(var_name, str):
        print("FATAL ERROR: The provided var_name ('{var_name}') is not a string."
                     + f" var_name must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not var_name:
        print("FATAL ERROR: The provided var_name is empty. var_name cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    if re.search(r'[^ A-Za-z0-9,_\-]', var_name):
        print("FATAL ERROR: The provided var_name string ('{var_name}') contains"
...
def check_VX_MASK_LIST(VX_MASK_LIST):
    if not isinstance(VX_MASK_LIST, str):
        print("FATAL ERROR: The provided VX_MASK_LIST ('{VX_MASK_LIST}') is not a string."
                     + f" VX_MASK_LIST must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not VX_MASK_LIST:
        print("FATAL ERROR: The provided VX_MASK_LIST is empty. VX_MASK_LIST cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    if re.search(r'[^ A-Za-z0-9,_\-]', VX_MASK_LIST):
        print("FATAL ERROR: The provided VX_MASK_LIST string ('{VX_MASK_LIST}') contains"
...
def check_FCST_LEAD(FCST_LEAD):
    if not isinstance(FCST_LEAD, str):
        print("FATAL ERROR: The provided FCST_LEAD ('{FCST_LEAD}') is not a string."
                     + f" FCST_LEAD must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not FCST_LEAD:
        print("FATAL ERROR: The provided FCST_LEAD is empty. FCST_LEAD cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    if re.search(r'[^ 0-9,]', FCST_LEAD):
        print("FATAL ERROR: The provided FCST_LEAD string ('{FCST_LEAD}') contains"
...
def check_LINE_TYPE(LINE_TYPE):
    if not isinstance(LINE_TYPE, str):
        print("FATAL ERROR: The provided LINE_TYPE ('{LINE_TYPE}') is not a string."
                     + f" LINE_TYPE must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not LINE_TYPE:
        print("FATAL ERROR: The provided LINE_TYPE is empty. LINE_TYPE cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    if re.search(r'[^ A-Za-z0-9]', LINE_TYPE):
        print("FATAL ERROR: The provided LINE_TYPE string ('{LINE_TYPE}') contains"
...
def check_INTERP(INTERP):
    if not isinstance(INTERP, str):
        print("FATAL ERROR: The provided INTERP ('{INTERP}') is not a string."
                     + f" INTERP must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not INTERP:
        print("FATAL ERROR: The provided INTERP is empty. INTERP cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    if re.search(r'[^A-Za-z0-9_\-]', INTERP):
        print("FATAL ERROR: The provided INTERP string ('{INTERP}') contains"
...
def check_FCST_THRESH(FCST_THRESH, LINE_TYPE):
    ...
    if str(LINE_TYPE).upper() in ['CTC','MCTC','PCT','NBRCTC']:
        ...
        if re.search(r'^((?![<=!>]).)*$', FCST_THRESH):
            print("FATAL ERROR: The provided FCST_THRESH string ('{FCST_THRESH}')"
...
def check_OBS_THRESH(OBS_THRESH, FCST_THRESH, LINE_TYPE):
    if not isinstance(OBS_THRESH, str):
        print("FATAL ERROR: The provided OBS_THRESH ('{OBS_THRESH}') is not a"
                     + f" string. OBS_THRESH must be a string. Check the"
                     + f" plotting configuration file.")
        sys.exit(1)
    if str(LINE_TYPE).upper() in ['CTC','MCTC','PCT','NBRCTC']:
        if not OBS_THRESH:
            print("FATAL ERROR: The provided OBS_THRESH is empty. Since the"
                         + f" provided line type is '{LINE_TYPE}', OBS_THRESH"
                         + f" cannot be empty. Check the plotting"
                         + f" configuration file.")
            sys.exit(1)
        if re.search(r'[^A-Za-z0-9<>=.,! /-]', OBS_THRESH):
            print("FATAL ERROR: The provided OBS_THRESH string ('{OBS_THRESH}') contains"
        ...
        if re.search(r'^((?![<=!>]).)*$', OBS_THRESH):
            print("FATAL ERROR: The provided OBS_THRESH string ('{OBS_THRESH}')"
                         + f" does not contain a valid comparison operator"
...
def check_STATS(STATS):
    if not isinstance(STATS, str):
        print("FATAL ERROR: The provided STATS ('{STATS}') is not a string."
                     + f" STATS must be a string. Check the plotting"
                     + f" configuration file.")
        sys.exit(1)
    if not STATS:
        print("FATAL ERROR: The provided STATS is empty. STATS cannot be"
                     + f" empty. Check the plotting configuration file.")
        sys.exit(1)
    if re.search(r'[^ A-Za-z0-9,_\-]', STATS):
        print("FATAL ERROR: The provided STATS string ('{STATS}') contains"
...
def check_CONFIDENCE_INTERVALS(CONFIDENCE_INTERVALS):
    if not isinstance(CONFIDENCE_INTERVALS, str):
        print("FATAL ERROR: The provided CONFIDENCE_INTERVALS ('{CONFIDENCE_INTERVALS}') is not a string."
