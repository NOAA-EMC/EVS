'''
Program Name: set_init_valid_fhr_subseasonal_info.py
...
# Build dictionary
env_var_dict = {}
if VCS in ['grid2grid_prep', 'grid2grid_stats', 'grid2grid_plots',
           'grid2obs_prep', 'grid2obs_stats', 'grid2obs_plots',
    ...
    # Process forecast initialization cycles
    VCS_abbrev_fcyc_list = os.environ[VCS_abbrev+'_fcyc_list'].split(' ')
    VCS_abbrev_fcyc_beg, VCS_abbrev_fcyc_end, VCS_abbrev_fcyc_inc = (
        get_hr_list_info(VCS_abbrev_fcyc_list)
    )
    env_var_dict[VCS_abbrev+'_fcyc_list'] = (
        os.environ[VCS_abbrev+'_fcyc_list']
    )
    env_var_dict[VCS_abbrev+'_init_hr_list'] = ', '.join(
        VCS_abbrev_fcyc_list
    )
    env_var_dict[VCS_abbrev+'_init_hr_beg'] = VCS_abbrev_fcyc_beg
    env_var_dict[VCS_abbrev+'_init_hr_end'] = VCS_abbrev_fcyc_end
    env_var_dict[VCS_abbrev+'_init_hr_inc'] = VCS_abbrev_fcyc_inc
    # Process valid hours
    VCS_abbrev_vhr_list = os.environ[VCS_abbrev+'_vhr_list'].split(' ')
    VCS_abbrev_vhr_beg, VCS_abbrev_vhr_end, VCS_abbrev_vhr_inc = (
        get_hr_list_info(VCS_abbrev_vhr_list)
    )
    env_var_dict[VCS_abbrev+'_vhr_list'] = (
        os.environ[VCS_abbrev+'_vhr_list']
    )
    env_var_dict[VCS_abbrev+'_valid_hr_list'] = ', '.join(
        VCS_abbrev_vhr_list
    )
    env_var_dict[VCS_abbrev+'_valid_hr_beg'] = VCS_abbrev_vhr_beg
    env_var_dict[VCS_abbrev+'_valid_hr_end'] = VCS_abbrev_vhr_end
    env_var_dict[VCS_abbrev+'_valid_hr_inc'] = VCS_abbrev_vhr_inc
    # Process forecast hours
    VCS_abbrev_fhr_min = os.environ[VCS_abbrev+'_fhr_min']
    VCS_abbrev_fhr_max = os.environ[VCS_abbrev+'_fhr_max']
    VCS_abbrev_fhr_list_str = get_forecast_hours(
        VCS_abbrev_fcyc_list, VCS_abbrev_vhr_list,
        VCS_abbrev_fhr_min, VCS_abbrev_fhr_max
    )
    env_var_dict[VCS_abbrev+'_fhr_list'] = VCS_abbrev_fhr_list_str
elif VCS in ['maps2d', 'mapsda']:
    for VCS_type in VCS_type_list:
        VCS_abbrev_type = VERIF_CASE_STEP_abbrev+'_'+VCS_type
        # Process hour information
        VCS_abbrev_type_make_met_data_by = (
            os.environ[VCS_abbrev_type+'_make_met_data_by']
        )
        VCS_abbrev_type_hr_list = (
            os.environ[VCS_abbrev_type+'_hour_list'].split(' ')
        )
        (VCS_abbrev_type_hr_beg,
         VCS_abbrev_type_hr_end,
         VCS_abbrev_type_hr_inc) = get_hr_list_info(VCS_abbrev_type_hr_list)
        env_var_dict[VCS_abbrev_type+'_hour_beg'] = VCS_abbrev_type_hr_beg
        env_var_dict[VCS_abbrev_type+'_hour_end'] = VCS_abbrev_type_hr_end
        env_var_dict[VCS_abbrev_type+'_hour_inc'] = VCS_abbrev_type_hr_inc
        env_var_dict[VCS_abbrev_type+'_make_met_data_by'] = (
            VCS_abbrev_type_make_met_data_by
        )
