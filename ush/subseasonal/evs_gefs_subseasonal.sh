...
if [ $passed_config_strlength = 0 ]; then
    export config=$HOMEevs/parm/evs_config/subseasonal/config.evs.gefs
...
echo "=============== Prepping Data ==============="
if [ $RUN_GRID2GRID_PREP = YES ] ; then
    echo
    echo "========== RUNNING GRID-TO-GRID PREP =========="
    echo "===== retrieving data for grid-to-grid verification ====="
    export VERIF_CASE="grid2grid"
    export STEP="prep"
    python $HOMEevs/ush/subseasonal/run_subseasonal_batch_prep.py
fi
...
echo "=============== Prepping Data ==============="
if [ $RUN_GRID2OBS_PREP = YES ] ; then
    echo
    echo "========== RUNNING GRID-TO-OBS PREP =========="
    echo "===== retrieving data for grid-to-obs verification ====="
