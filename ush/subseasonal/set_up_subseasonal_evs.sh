...
echo "BEGIN: $(basename S{BASH_SOURCE[0]})"
...
python $HOMEevs/ush/subseasonal/subseasonal_get_machine.py
status=$?
[[ $status -ne 0 ]] && exit $status
[[ $status -eq 0 ]] && echo "Successfully ran subseasonal_get_machine.py"
echo
...
if [[ "$machine" =~ ^(HERA|ORION|WCOSS2)$ ]]; then
   echo
else
...
. $HOMEevs/ush/subseasonal/subseasonal_load_modules.sh
status=$?
[[ $status -ne 0 ]] && exit $status
...
echo "END: $(basename S{BASH_SOURCE[0]})"
