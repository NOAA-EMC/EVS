...
#### EMC-verif_global title creation script
...
plt.rcParams['figure.subplot.top'] = 0.85 #0.925
plt.rcParams['figure.subplot.bottom'] = 0.15 #0.075
plt.rcParams['legend.handletextpad'] = 0.25
plt.rcParams['legend.handlelength'] = 1.25
plt.rcParams['legend.borderaxespad'] = 0
plt.rcParams['legend.columnspacing'] = 1.0
plt.rcParams['legend.frameon'] = True
x_figsize, y_figsize = 14, 7 #14
legend_bbox_x, legend_bbox_y = 0.5, 0.05
legend_fontsize = 13 #15
legend_loc = 'center'
legend_ncol = 5
title_loc = 'center'
...
noaa_logo_ypixel_loc = y_figsize*plt.rcParams['figure.dpi']*0.865 #0.9325
noaa_logo_alpha = 0.5
...
nws_logo_ypixel_loc = y_figsize*plt.rcParams['figure.dpi']*0.865 #0.9325
nws_logo_alpha = 0.5
...
output_imgs_dir = os.path.join(output_base_dir, 'images')
# Model info
...
#### EMC-verif_global box widths
fhrs = np.asarray(os.environ['fhr_list'].split(', ')).astype(float)
fhrs_sec = fhrs * 3600.
CI_bar_max_widths = np.append(np.diff(fhrs_sec),
                              fhrs_sec[-1]-fhrs_sec[-2])/1.5
CI_bar_min_widths = np.append(np.diff(fhrs_sec),
                              fhrs_sec[-1]-fhrs_sec[-2])/nmodels
CI_bar_intvl_widths = (
    (CI_bar_max_widths-CI_bar_min_widths)/nmodels
)
#CI_bar_intvl_widths = CI_bar_intvl_widths
#CI_bar_max_widths = CI_bar_max_widths
#CI_bar_min_widths = CI_bar_min_widths
# Plot info
plot_info_list = list(
...
for plot_info in plot_info_list:
    ...
    for stat in stats_list:
        ...
        for model_info in model_info_list:
            ...
            if model_num == 1:
                fig, ax1 = plt.subplots(1,1,figsize=(x_figsize, y_figsize))
                #fig, (ax1, ax2) = plt.subplots(2, 1,
                                               #figsize=(x_figsize, y_figsize),
                                               #sharex=True)
                ax1.grid(True)
                if len(fcst_lead_timedeltas) >= 15:
                    ax1.set_xticks(fcst_lead_timedeltas[::2])
                    ax1.set_xticklabels(fcst_lead_timedeltas_str[::2])
                else:
                ...
                props = {'boxstyle': 'square',
                         ...
                         'linewidth': 1,
                         'edgecolor': 'black'}
                #ax2.text(0.995, 1.05, 'Note: differences outside the '
            ...
            mfcst_lead_timedeltas = np.ma.array(
                fcst_lead_timedeltas,
                mask=np.ma.getmaskarray(model_avg_data[0,:])
            )
            if count1 != 0:
                ax1.plot(mfcst_lead_timedeltas.compressed(),
            ...
            #)
                ...
                #ax2.plot(mfcst_lead_timedeltas.compressed(),
                         #(model_avg_data[0,:] - diff_from_avg_data) \
                         #.compressed(),
                         #color = model_plot_settings_dict['color'],
                         #linestyle = model_plot_settings_dict['linestyle'],
            ...
            #)
                ...
                #bottom_bar_data_min = model_CI_data.max() * -1
                        ...
                        #stat_min_max_dict['ax2_stat_max'] = (
                            #top_bar_data_max
                        #)
                #for fcst_lead_timedelta in fcst_lead_timedeltas:
                    #idx = np.where(
                        #fcst_lead_timedeltas == fcst_lead_timedelta
                    #)[0][0]
                    #ax2.bar(fcst_lead_timedeltas[idx],
                            ...
                            #linewidth=1)
        #### EMC_verif-global adjust y axis
        subplot_num = 1
        for ax in fig.get_axes():
           ...
           if stat_min <= ax.get_ylim()[0]:
               while stat_min <= ax.get_ylim()[0]:
                   ...
                   ax.set_ylim([y_axis_min, y_axis_max])
           subplot_num+=1
        #### EMC-verif-global add legend, adjust if points in legend
        if len(ax1.lines) != 0:
