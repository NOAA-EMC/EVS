'''
Program Name: create_METplus_subseasonal_job_scripts.py
...
def init_env_dict():
    ...
    """
    env_var_list = [
        'HOMEevs', 'USHevs', 'PARMevs', 'MET_ROOT',
        'FIXevs', 'members', 'MODELNAME', 'COMINgefs', 'COMINcfs', 
        'OBSNAME', 'COMINobs', 'COMINclimo',
        'METplus_version', 'METPLUS_PATH',
        'log_MET_output_to_METplus', 'METplus_verbosity', 'MET_version',
        'MET_bin_exec', 'MET_verbosity', 'DATA', 'RUN', 'RM', 'USE_CFP',
        'NET', 'COMPONENT', 'STEP', 'VERIF_CASE', 'VERIF_CASE_STEP', 'nproc',
        'config', 'CUT', 'TR', 'NCAP2', 'CONVERT', 'NCDUMP', 'envir'
    ]
    env_var_dict = {}
...
def create_job_scripts_stats(start_date_dt, end_date_dt, case, case_abbrev,
    ...
    # Set case
    job_env_dict['VERIF_CASE_STEP_case'] = case
    ...
    for model in os.environ['model_list'].split(' '):
        ...
        for case_type in case_type_list:
            job_env_dict['VERIF_CASE_STEP_type'] = case_type
            ...
            while date_dt <= end_date_dt:
                ...
                if case == 'grid2grid':
                    truth_file_list = glob.glob(
                        os.path.join(job_env_dict['COMINobs'],
                                     OBSNAME+'.'
                ...
                if case == 'grid2grid':
                   metplus_conf_list = [
                      os.path.join(
                         ...
                         'GridStat_fcst'+model.upper()+'_obs'
                         +obtype+'_climo'
                         +'ERA5_'+case_type+'.conf'
                      )
                   ]
                   if case_type == 'precip':
                      metplus_conf_list = [
                     ...
                     metplus_conf_list = [
                        os.path.join(
                           make_met_data_conf_dir,
                           'PointStat_fcst'+model.upper()+'_obs'
                           +obtype+'_climo'
                           +'ERA5_'
                           +case_type+'.conf'
                        )
                ...
                for vx_mask in vx_mask_list:
                    ...
                    job_file.write(
                        ...
                        +os.path.join(job_env_dict['USHevs'],
                                      'subseasonal',
                                      'prune_subseasonal_stat_files.py\n')
                    )
                    job_file.write('\n')
                    ...
                    line_type_var_name_vx_mask_img_dir = os.path.join(
                        job_env_dict['DATA'], job_env_dict['VERIF_CASE_STEP'],
                        'metplus_output',
                    ...
                    if case == 'maps2d' \
                        ...
                        job_file.write(
                            ...
                            +os.path.join(
                                job_env_dict['USHevs'],
                               'subseasonal', 
                               'create_MET_series_anl_subseasonal_jobs.py')
                               +'\n'
                        )
                        for model in model_list:
...
edate = datetime.datetime(int(end_date[0:4]), int(end_date[4:6]),
                          int(end_date[6:]))
# Set important METplus paths
USHMETplus_run_metplus = os.path.join(os.environ['METPLUS_PATH'], 'ush',
                                      'run_metplus.py')
PARMevs_machine_conf = os.path.join(os.environ['PARMevs'], 'metplus_config', 
                                    'machine.conf')
...
if STEP == 'stats':
    create_job_scripts_stats(
        sdate, edate, VERIF_CASE, VERIF_CASE_STEP_abbrev,
    ...
    create_job_scripts_tropcyc(
        sdate, edate, VERIF_CASE, RUN_abbrev, RUN_abbrev_tc_list,
        USHMETplus_run_metplus, PARMevs_machine_conf,
        PARMevs_METplus_conf_dir
    )
elif VERIF_CASE in ['maps2d', 'mapsda']:
    create_job_scripts_maps(
        sdate, edate, VERIF_CASE, RUN_abbrev,
...
if USE_CFP == 'YES':
    job_files = glob.glob(
        os.path.join(DATA, VERIF_CASE_STEP, 'metplus_job_scripts', 'job*')
    )
    ...
    while njob <= njob_files:
        ...
        poe_file.write(
            os.path.join(DATA, VERIF_CASE_STEP, 
                         'metplus_job_scripts', job)+'\n'
        )
        njob+=1
    poe_file.close()
    # If at final record and have not reached the
    # final processor then write echos to
