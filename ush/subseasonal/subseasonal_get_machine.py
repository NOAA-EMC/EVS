'''
Program Name: subseasonal_get_machine.py
Contact(s): Shannon Shields
Abstract: This script is run by set_up_subseasonal_evs.sh.
...
if 'machine' not in vars():
    ...
    cactus_match2 = re.match(re.compile(r"^cdecflow[0-9]{2}$"), hostname)
    dogwood_match = re.match(re.compile(r"^dlogin[0-9]{2}$"), hostname)
    dogwood_match2 = re.match(re.compile(r"^ddecflow[0-9]{2}$"), hostname)
    if hera_match:
        machine = 'HERA'
    elif orion_match:
        machine = 'ORION'
