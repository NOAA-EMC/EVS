#!/bin/ksh
#************************************************************
# Purpose: collect small stat files to form a big stat file
#          by using MET StatAnalysis tool
# Input parameters:
#     modl   - model name 
...
for modnam in $models ; do
   ...
   >run_gather_${modnam}_${verify}.sh
    ...
    if [ $nsmall_stat_files -eq 0 ]; then
      err_exit "No small stat files in ${COMOUTsmall}"
    fi
    if [ $verify = grid2obs ] ; then   
      echo "${METPLUS_PATH}/ush/run_metplus.py -c ${PARMevs}/metplus_config/machine.conf -c ${GRID2OBS_CONF}/StatAnalysis_fcstGENS_obsPREPBUFR_GatherByDay.conf" >> run_gather_${modnam}_${verify}.sh
    elif [ $verify = grid2grid ] ; then
      echo "${METPLUS_PATH}/ush/run_metplus.py -c ${PARMevs}/metplus_config/machine.conf -c ${GRID2GRID_CONF}/StatAnalysis_fcstGENS_obsAnalysis_GatherByDay.conf" >> run_gather_${modnam}_${verify}.sh
    elif [ $verify = precip ] ; then
      echo "${METPLUS_PATH}/ush/run_metplus.py -c ${PARMevs}/metplus_config/machine.conf -c ${GRID2GRID_CONF}/StatAnalysis_fcstGENS_obsCCPA_GatherByDay.conf" >> run_gather_${modnam}_${verify}.sh
    fi
