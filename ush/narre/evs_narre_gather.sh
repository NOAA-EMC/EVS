...
for verify in $verifys   ; do 
  ...
  >run_gather_${verify}.sh
    ...
    echo "[[ $SENDCOM = "YES" ]] && [[ -s ${WORK}/gather/${vday}/${MODEL}_MEAN_grid2obs_${vday}.stat ]] && cp ${WORK}/gather/${vday}/${MODEL}_MEAN_grid2obs_${vday}.stat  $COMOUTfinal/evs.stats.${model}.mean.grid2obs.v${vday}.stat">>run_gather_${verify}.sh
