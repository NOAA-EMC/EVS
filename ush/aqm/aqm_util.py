...
def get_daily_stat_file(model_name, source_stats_base_dir,
    ...
    """! Link model daily stat files
         Args:
             ...
             source_stats_base_dir     - full path to stats/aqm
   ...
   if stat == 'ACC': # Anomaly Correlation Coefficient
       ...
       if line_type == 'SL1L2':
           ...
           for irec in range(0,icnt):
               if irec == 0 :
                   validtime_ref=validtime[irec]
                   biasme_sum=radicand[irec]
                   nrec=1
               elif irec == icnt-1:                  ## last record
                   biasme_sum=biasme_sum+radicand[irec]
                   nrec+=1
       ...
       if line_type == 'SL1L2':
           ...
           radicand = var_f*var_o
           radicand[radicand<0] = np.nan
           radicand2 = (FOBAR - (FBAR*OBAR))/np.sqrt(radicand)
           CORR_MEAN=[]
           icnt=len(validtime)
           for irec in range(0,icnt):
               if irec == 0 :
                   validtime_ref=validtime[irec]
                   corr_sum=radicand2[irec]
                   nrec=1
               elif irec == icnt-1:                  ## last record
                   corr_sum=corr_sum+radicand2[irec]
                   nrec+=1
       ...
       if line_type == 'CTC':
           radicand = FY_OY/(FY_OY + FY_ON + FN_OY)
           CSI_MEAN=[]
           icnt=len(validtime)
           for irec in range(0,icnt):
               if irec == 0 :
                   validtime_ref=validtime[irec]
                   csi_sum=radicand[irec]
                   nrec=1
               elif irec == icnt-1:                  ## last record
                   csi_sum=csi_sum+radicand[irec]
                   nrec+=1
       ...
       if line_type == 'CTC':
           radicand = (FY_OY + FY_ON)/(FY_OY + FN_OY)
           FBIAS_MEAN=[]
           icnt=len(validtime)
           for irec in range(0,icnt):
               if irec == 0 :
                   validtime_ref=validtime[irec]
                   fbias_sum=radicand[irec]
                   nrec=1
               elif irec == icnt-1:                  ## last record
                   fbias_sum=fbias_sum+radicand[irec]
                   nrec+=1
           ...
           for irec in range(0,icnt):
               if irec == 0 :
                   validtime_ref=validtime[irec]
                   fbias_sum=FBIAS[irec]
                   nrec=1
               elif irec == icnt-1:                  ## last record
                   fbias_sum=fbias_sum+FBIAS[irec]
                   nrec+=1
                   fbias_mean=fbias_sum/float(nrec)
                   FBIAS_MEAN.append(fbias_mean)
               else:
       ...
       if line_type == 'CTC':
           ...
           for irec in range(0,icnt):
               if irec == 0 :
                   validtime_ref=validtime[irec]
                   pod_sum=radicand[irec]
                   nrec=1
               elif irec == icnt-1:                  ## last record
                   pod_sum=pod_sum+radicand[irec]
                   nrec+=1
       ...
       if line_type == 'SL1L2':
           ...
           for irec in range(0,icnt):
               if irec == 0 :
                   validtime_ref=validtime[irec]
                   rmse_sum=radicand[irec]
                   nrec=1
               elif irec == icnt-1:                  ## last record
                   rmse_sum=rmse_sum+radicand[irec]
                   nrec+=1
       ...
       if line_type == 'CTC':
           SRATIO = 1 - (FY_ON/(FY_ON + FY_OY))
           SRATIO_MEAN=[]
           icnt=len(validtime)
           for irec in range(0,icnt):
               if irec == 0 :
                   validtime_ref=validtime[irec]
                   sratio_sum=SRATIO[irec]
                   nrec=1
               elif irec == icnt-1:                  ## last record
                   sratio_sum=sratio_sum+SRATIO[irec]
                   nrec+=1
