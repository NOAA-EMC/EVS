...
class Presets():
    def __init__(self):
        ...
        self.date_presets = {
            'LAST90DAYS': {
                'valid_beg': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=90)
                ).strftime('%Y%m%d'),
                'valid_end': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=1)
                ).strftime('%Y%m%d'),
                'init_beg': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=90)
                    ).strftime('%Y%m%d'),
            ...
            'LAST31DAYS': {
                'valid_beg': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=31)
                ).strftime('%Y%m%d'),
                'valid_end': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=1)
                ).strftime('%Y%m%d'),
            ...
            'LAST30DAYS': {
                'valid_beg': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=30)
                ).strftime('%Y%m%d'),
                'valid_end': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=1)
                ).strftime('%Y%m%d'),
            ...
            'LAST7DAYS': {
                'valid_beg': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=7)
                ).strftime('%Y%m%d'),
                'valid_end': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=1)
                ).strftime('%Y%m%d'),
            ...
            'LAST3DAYS': {
                'valid_beg': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=3)
                ).strftime('%Y%m%d'),
                'valid_end': (
                    datetime.strptime(os.environ['VDATE_END'], '%Y%m%d')-td(days=1)
                ).strftime('%Y%m%d'),
...
class Reference():
    def __init__(self):
        ...
        self.variable_translator = {'TMP': 'Temperature',
                                    ...
                                    'TOZNE': 'Total Ozone',
                                    'OZCON1': 'OZCON1',
                                    'OZMAX8': 'Daily Max 8-hr average ozone',
                                    'OZONE-8HR': 'Daily Max 8-hr average ozone',
                                    'PMAVE': '24-hr average PM2.5',
                                    'PM2.5-24hr': '24-hr average PM2.5',
                ...
                'CTC': {
                    ...
                    'var_dict': {
                        ...
                        'OZCON8': {'fcst_var_names': ['OZCON8'],
                                  ...
                                  'obs_var_options': '',
                                  'plot_group':'aq'},
                         'OZMAX8': {'fcst_var_names': ['OZMAX8'],
                                    'fcst_var_levels': ['L1'],
                                    'fcst_var_thresholds': ('>50, >60, >65, >70,'
                                                            + '>75, >85, >105,'
                                                            + '>125, >150'),
                                    'fcst_var_options': '',
                                    'obs_var_names': ['OZONE-8HR'],
                                    'obs_var_levels': ['A8'],
                                    'obs_var_thresholds': ('>50, >60, >65, >70,'
                                                           + '>75, >85, >105,'
                                                           + '>125, >150'),
