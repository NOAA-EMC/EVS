...
# Modified:      Jun. 3, 2022 by Marcel Caron
#                Nov. 15, 2022 by L. Gwen Chen (lichuan.chen@noaa.gov)
# Title:         Performance diagram of verification metrics as a function of 
#                forecast threshold
# Abstract:      Plots METplus output (e.g., POD, CSI) as a diagram, varying 
#                by forecast threshold. Line colors and styles are unique for
#                each model, and several models can be plotted at once.
#
###############################################################################
...
def plot_performance_diagram(df: pd.DataFrame, logger: logging.Logger, 
    ...
    if isinstance(flead, list):
        ...
        frange_string = (f'Forecast Hours {flead[0]:03d}'+u'\u2013'
                         + f'{flead[1]:03d}')
    ...
    save_name = (f'evs.'
                 ...
                 + f'perfdiag_{str(date_type).lower()}{str(date_hours_savename).lower()}_'
